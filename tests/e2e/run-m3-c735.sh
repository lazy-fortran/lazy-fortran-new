#!/usr/bin/env bash
# Run the bounded M3 C735 derived-type attribute uniqueness contract.
...
if [ "$#" -eq 1 ] && [ "$1" = "--fresh" ]; then
    run_number=1
    while :; do
        run_arg=".cache/runs/E0202/R$(printf '%06d' "$run_number")"
    ...
    case "$run_arg" in
        .cache/runs/E0202/*) ;;
        *) die "run directory must be below E0202" ;;
    esac
    run_dir="$ROOT/$run_arg"
else
    die "usage: $0 .cache/runs/E0202/R<fresh-run>|--fresh"
fi
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c735-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c735-semantic-items.sx"
...
golden="$ROOT/tests/golden/m3-c735-semantic-items.sx"
validator="$ROOT/tests/e2e/validate_m3_c735.py"
manifest="$ROOT/research/experiments/E0202-can-a-deterministic-source-backed-c735-oracle/manifest.yaml"
standard="$(resolve_repo standard-new)"

central_commit="$(git -C "$ROOT" rev-parse HEAD)"
standard_commit="$(git -C "$standard" rev-parse HEAD)"
expected_central_commit="${M3_C735_EXPECTED_CENTRAL_COMMIT:-}"
...
functional_paths=(
    contracts/m3-c735-derived-type-attr-uniqueness-oracle-v0.sxs
    contracts/fixtures/m3-c735-derived-type-attr-uniqueness-oracle-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c735.sh
    tests/e2e/validate_m3_c735.py
    tests/fixtures/m3-c735-semantic-items.sx
    tests/fixtures/m3-c735-source-backed-v0.json
    tests/golden/m3-c735-semantic-items.sx
    artifacts/traces/m3-c735-source-backed-v0.json
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || die "functional C735 files differ from manifest central pin $central_pin"
...
[ -f "$ROOT/artifacts/traces/m3-c735-source-backed-v0.json" ] || die "missing committed C735 trace"
cmp "$run_dir/result.json" "$ROOT/artifacts/traces/m3-c735-source-backed-v0.json"
...
record = {
    ...
    "commands": ["M3_C735_EXPECTED_CENTRAL_COMMIT=<pinned-central-revision> tests/e2e/run-m3-c735.sh --fresh", "scripts/check_pins.sh", "scripts/check-contracts.sh", "scripts/check-contracts.sh --self-test", "scripts/fetch.sh j3-24-007", "(cd standard-new && fo clean)", "(cd standard-new && fo)", "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c735-semantic-items.sx <run-dir>/semantic-items.canonical.sx)", "python3 tests/e2e/validate_m3_c735.py --self-test", "python3 tests/e2e/validate_m3_c735.py ..."],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C735 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C735 gate"
printf 'M3 C735 PASS: %s\n' "$run_dir"
