#!/usr/bin/env python3
"""Independent validator for the bounded M3 C719 semantic slice."""
...
PROPERTY = "kind-param-nonnegative-legality"
EXPECTED_CANONICAL_LINES = [
    {"line": 3297, "text": "C719 (R709) The value of kind-param shall be nonnegative."}
]
EXPECTED_STANDARDIR_ROWS = [
    {"rule": "R709", "lhs": "kind-param", "page": 80, "byte_start": 207647, "byte_length": 65, "occurrence": 59},
]
...
def c719_oracle(candidate: dict[str, Any]) -> str:
    """Apply C719 to a typed presence/value state."""

    kind_param = candidate["kind_param"]
    value = candidate["value"]
    if kind_param == "unknown" or value == "unknown":
        return "UNRESOLVED"
    if kind_param == "absent" and value == "not-applicable":
        return "ACCEPTED"
    if kind_param == "present" and value == "nonnegative":
        return "ACCEPTED"
    if kind_param == "present" and value == "negative":
        return "REJECTED"
    return "REJECTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "kind_param", "value"}, f"case {case['id']} candidate")
    require(candidate["fact"] == "kind-param-value-use", f"case {case['id']} candidate fact differs")
    require(candidate["source_rule"] == "C719", f"case {case['id']} source rule differs")
    require(candidate["kind_param"] in {"absent", "present", "unknown"}, f"case {case['id']} kind-param state is invalid")
    require(candidate["value"] in {"nonnegative", "negative", "not-applicable", "unknown"}, f"case {case['id']} value state is invalid")
    if candidate["kind_param"] == "absent":
        require(candidate["value"] == "not-applicable", f"case {case['id']} absent kind-param has a value")
    else:
        require(candidate["value"] != "not-applicable", f"case {case['id']} present/unknown kind-param lacks a value state")
    computed = c719_oracle(candidate)
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(record kind-param-value-use" in schema, "contract schema lacks typed kind-param fact")
    require("(kind-param kind-param-state)" in schema, "contract schema lacks kind-param state")
    require("(value kind-param-value-state)" in schema, "contract schema lacks value state")
...
def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in (
        "(contract m3-c719-kind-param-nonnegative-oracle)",
        "(property kind-param-nonnegative-legality)",
        "(document J3-24-007)",
        "(clause 7)",
        "(rule C719)",
...
def validate_source_binding(doc: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, standardir_path: Path, semantic_input: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c719-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C719", "source rule differs")
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, standardir_path: Path, semantic_input: Path, semantic_output: Path, golden: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c719.py",
    }


def self_test() -> None:
    cases = [
        ({"fact": "kind-param-value-use", "source_rule": "C719", "kind_param": "absent", "value": "not-applicable"}, "ACCEPTED"),
        ({"fact": "kind-param-value-use", "source_rule": "C719", "kind_param": "present", "value": "nonnegative"}, "ACCEPTED"),
        ({"fact": "kind-param-value-use", "source_rule": "C719", "kind_param": "present", "value": "negative"}, "REJECTED"),
        ({"fact": "kind-param-value-use", "source_rule": "C719", "kind_param": "present", "value": "unknown"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c719_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C719 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    if argv[1:] == ["--self-test"]:
        ...
        return 0
    if len(argv) != 8:
        print("usage: validate_m3_c719.py FIXTURE SEMANTIC_OUTPUT STANDARDIR CANONICAL_TEXT PDF GOLDEN RESULT_JSON", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"M3 C719 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C719 validator: PASS")
    return 0
