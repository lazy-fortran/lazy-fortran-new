...
oracle="$ROOT/tests/e2e/oracle_generated_print_expression_multiply.py"
run_dir="$(mktemp -d)"
...
run_positive() {
    ...
    if python3 "$oracle" "$mutated_ast" "$mir" "$elf" "$source" >/dev/null 2>&1; then
        printf 'multiply AST mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
    sed '0,/(opcode mul)/s//(opcode load)/' "$mir" > "$mutated_mir"
    if python3 "$oracle" "$ast" "$mutated_mir" "$elf" "$source" >/dev/null 2>&1; then
        printf 'multiply MIR mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
    cp "$elf" "$mutated_elf"
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'multiply ELF mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'multiply negative accepted: %s\n' "$source" >&2
        exit 1
    fi
    [ ! -e "$ast" ] || {
        printf 'multiply negative wrote AST: %s\n' "$source" >&2
        exit 1
    }
}

run_positive "$ROOT/tests/fixtures/l3-print-expression-multiply-v0.f90" expression-multiply-first
run_positive "$ROOT/tests/fixtures/l3-print-expression-multiply-wide-v0.f90" expression-multiply-second
run_negative "$ROOT/tests/negative/l3-print-expression-multiply-v0-missing-operand.f90" missing-operand
run_negative "$ROOT/tests/negative/l3-print-expression-multiply-v0-wrong-operator.f90" wrong-operator
run_negative "$ROOT/tests/negative/l3-print-expression-multiply-v0-write.f90" write
run_negative "$ROOT/tests/negative/l3-print-expression-multiply-v0-wrong-name.f90" wrong-name

printf '%s\n' 'generic multiply-expression PASS'
