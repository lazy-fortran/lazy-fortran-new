...
oracle="$ROOT/tests/e2e/oracle_generated_print_expression_power_four.py"
run_dir="$(mktemp -d)"

bash "$ROOT/scripts/check-contracts.sh" >/dev/null
bash "$ROOT/scripts/check_pins.sh" >/dev/null
bash "$ROOT/tests/e2e/check-generated-print-expression-power-three.sh" >/dev/null
...
run_positive() {
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'power-four ELF mutation was accepted: %s\n' "$source" >&2; exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'power-four negative accepted: %s\n' "$source" >&2; exit 1
    fi
    [ ! -e "$ast" ] || { printf 'power-four negative wrote AST: %s\n' "$source" >&2; exit 1; }
}

run_positive "$ROOT/tests/fixtures/l3-print-expression-power-four-v0.f90" expression-power-four-first
run_positive "$ROOT/tests/fixtures/l3-print-expression-power-four-wide-v0.f90" expression-power-four-second
run_negative "$ROOT/tests/negative/l3-print-expression-power-four-v0-missing-operand.f90" missing-operand
run_negative "$ROOT/tests/negative/l3-print-expression-power-four-v0-wrong-value.f90" wrong-value
run_negative "$ROOT/tests/negative/l3-print-expression-power-four-v0-write.f90" write
run_negative "$ROOT/tests/negative/l3-print-expression-power-four-v0-wrong-name.f90" wrong-name

printf '%s\n' 'generic power-four-expression PASS'
