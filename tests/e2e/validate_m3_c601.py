#!/usr/bin/env python3
"""Independent validator for the bounded M3 C601 semantic slice."""
...
def c601_oracle(candidate: dict[str, Any]) -> str:
    """Apply R601--R603 shape, then the C601 maximum of 63 characters."""

    spelling = candidate["spelling"]
    if NAME_RE.fullmatch(spelling) is None:
        return "UNRESOLVED"
    return "ACCEPTED" if len(spelling) <= 63 else "REJECTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    require(candidate["type"] == "name-length-use", f"case {case['id']} candidate type differs")
    require(candidate["source_rule"] == "C601", f"case {case['id']} source rule differs")
    require_string(candidate["spelling"], f"case {case['id']} spelling")
    computed = c601_oracle(candidate)
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(record name-length-use" in schema, "contract schema lacks typed name fact")
...
def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in (
        "(contract m3-c601-name-length-oracle)",
        "(property name-length-legality)",
        "(document J3-24-007)",
        "(clause 6)",
        "(rule C601)",
...
def validate_source_binding(
    ...
    require(doc["schema_version"] == "m3-c601-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C601", "source rule differs")
    require(source["printed_page"] == 67, "source printed page differs")
...
def build_result(
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c601.py",
    }


def self_test() -> None:
    cases = [
        ({"type": "name-length-use", "source_rule": "C601", "spelling": "A"}, "ACCEPTED"),
        ({"type": "name-length-use", "source_rule": "C601", "spelling": "A" * 63}, "ACCEPTED"),
        ({"type": "name-length-use", "source_rule": "C601", "spelling": "A" * 64}, "REJECTED"),
        ({"type": "name-length-use", "source_rule": "C601", "spelling": "1A"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c601_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C601 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 8:
        print(
            "usage: validate_m3_c601.py FIXTURE SEMANTIC_OUTPUT STANDARDIR "
    ...
    try:
        ...
        print(f"M3 C601 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C601 validator: PASS")
    return 0
