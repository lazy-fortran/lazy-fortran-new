#!/usr/bin/env bash
# Run the bounded M3 C601 semantic-oracle delivery contract.
...
[ "$#" -eq 1 ] || die "usage: $0 .cache/runs/E0177/R<fresh-run>"
run_arg="$1"
case "$run_arg" in
    .cache/runs/E0177/*) ;;
    *) die "run directory must be below .cache/runs/E0177" ;;
esac
run_dir="$ROOT/$run_arg"
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c601-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c601-semantic-items.sx"
...
golden="$ROOT/tests/golden/m3-c601-semantic-items.sx"
validator="$ROOT/tests/e2e/validate_m3_c601.py"
manifest="$ROOT/research/experiments/E0177-can-a-deterministic-oracle-enforce-c601-/manifest.yaml"
...
functional_paths=(
    ...
    contracts/fixtures/m3-c601-name-length-oracle-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c601.sh
    tests/e2e/validate_m3_c601.py
    tests/fixtures/m3-c601-semantic-items.sx
    tests/fixtures/m3-c601-source-backed-v0.json
    tests/golden/m3-c601-semantic-items.sx
    artifacts/traces/m3-c601-source-backed-v0.json
    research/decisions/D0127-third-m3-c601-name-length-oracle.md
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || \
    die "functional M3 C601 files differ from manifest central pin $central_pin"
...
python3 "$validator" "$fixture" "$run_dir/semantic-items.canonical.sx" "$standardir" \
    ...
    >"$run_dir/oracle.log"
cmp "$run_dir/result.json" "$ROOT/artifacts/traces/m3-c601-source-backed-v0.json"
...
record = {
    ...
    "inputs": {
        "fixture": {"path": "tests/fixtures/m3-c601-source-backed-v0.json", "sha256": digest(fixture)},
        "semantic_items": {"path": "tests/fixtures/m3-c601-semantic-items.sx", "sha256": digest(semantic_input)},
    ...
    "commands": [
        ...
        "(cd standard-new && fo)",
        "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c601-semantic-items.sx <run-dir>/semantic-items.canonical.sx)",
        "python3 tests/e2e/validate_m3_c601.py --self-test",
        "python3 tests/e2e/validate_m3_c601.py ...",
    ],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || \
    die "standard-new became dirty during the M3 C601 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || \
    die "central checkout became dirty during the M3 C601 gate"

printf 'M3 C601 PASS: %s\n' "$run_dir"
