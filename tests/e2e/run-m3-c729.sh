#!/usr/bin/env bash
# Run the bounded M3 C729 semantic-oracle delivery contract.
...
fixture="$ROOT/tests/fixtures/m3-c729-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c729-semantic-items.sx"
...
golden="$ROOT/tests/golden/m3-c729-semantic-items.sx"
validator="$ROOT/tests/e2e/validate_m3_c729.py"
manifest="$ROOT/research/experiments/E0184-can-a-deterministic-oracle-enforce-c729-/manifest.yaml"
...
functional_paths=(
    ...
    contracts/fixtures/m3-c729-optional-comma-oracle-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c729.sh
    tests/e2e/validate_m3_c729.py
    tests/fixtures/m3-c729-semantic-items.sx
    tests/fixtures/m3-c729-source-backed-v0.json
    tests/golden/m3-c729-semantic-items.sx
    artifacts/traces/m3-c729-source-backed-v0.json
    research/decisions/D0133-ninth-m3-c729-optional-comma-oracle.md
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || die "functional M3 C729 files differ from manifest central pin $central_pin"
...
cmp "$run_dir/result.json" "$ROOT/artifacts/traces/m3-c729-source-backed-v0.json"
...
record = {
    ...
    "inputs": {
        "fixture": {"path": "tests/fixtures/m3-c729-source-backed-v0.json", "sha256": digest(fixture)},
        "semantic_items": {"path": "tests/fixtures/m3-c729-semantic-items.sx", "sha256": digest(semantic_input)},
    ...
    "commands": ["scripts/check_pins.sh", "scripts/check-contracts.sh", "scripts/check-contracts.sh --self-test", "scripts/fetch.sh j3-24-007", "(cd standard-new && fo clean)", "(cd standard-new && fo)", "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c729-semantic-items.sx <run-dir>/semantic-items.canonical.sx)", "python3 tests/e2e/validate_m3_c729.py --self-test", "python3 tests/e2e/validate_m3_c729.py ..."],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C729 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C729 gate"
printf 'M3 C729 PASS: %s\n' "$run_dir"
