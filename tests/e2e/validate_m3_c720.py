#!/usr/bin/env python3
"""Independent validator for the bounded M3 C720 semantic slice."""
...
PROPERTY = "kind-param-representation-method-legality"
...
def c720_oracle(candidate: dict[str, Any]) -> str:
    """Apply only the C720 representation-method relation."""

    return {
        "present": "ACCEPTED",
        "absent": "REJECTED",
        "unknown": "UNRESOLVED",
    }[candidate["representation_method"]]
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "representation_method"}, f"case {case['id']} candidate")
    require(candidate["fact"] == "kind-param-representation-method-use", f"case {case['id']} candidate fact differs")
    require(candidate["source_rule"] == "C720", f"case {case['id']} source rule differs")
    require(candidate["representation_method"] in {"absent", "present", "unknown"}, f"case {case['id']} representation-method state is invalid")
    computed = c720_oracle(candidate)
    require(computed == case["expected"], f"case {case['id']} expected outcome disagrees with oracle")
    return {"id": case["id"], "kind": case["kind"], "representation_method": candidate["representation_method"], "computed": computed, "expected": case["expected"]}
...
def validate_standardir_rows(source: dict[str, Any], standardir_path: Path) -> None:
    ...
    require(standardir_spec["rows"] == [{"rule": "R708", "lhs": "int-literal-constant", "page": 80, "byte_start": 207585, "byte_length": 61, "occurrence": 58}], "StandardIR row identity differs")
...
def validate_canonical_lines(source: dict[str, Any], canonical_path: Path) -> None:
    ...
    expected = [{"line": 3298, "text": "C720 (R708) The value of kind-param shall specify a representation method that exists on the processor."}]
    ...
    for item in source["canonical_lines"]:
        ...
        require(len(parts) == 2 and parts[0].isdigit(), f"canonical source lacks line marker: {item['line']}")
...
def validate_page_index(source: dict[str, Any], page_index_path: Path) -> None:
    ...
    expected = {"path": ".cache/runs/E0001/R000003/j3-24-007.pages.index", "sha256": PAGE_INDEX_SHA256, "page": 80, "start": 204806, "length": 2920}
    require(source["page_index"] == expected, "page index identity differs")
    require("page 80 start 204806 length 2920" in page_index_path.read_text(encoding="utf-8"), "page index span differs")
...
def validate_contract(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in ("(enum outcome accepted rejected unresolved)", "(enum representation-method-state absent present unknown)", "(record kind-param-representation-method-use", "(record semantic-candidate"):
        require(fragment in schema, f"contract schema lacks {fragment}")
    witness = (root / doc["contract"]["fixture"]).read_text(encoding="utf-8")
    for fragment in ("(contract m3-c720-kind-param-representation-oracle)", "(property kind-param-representation-method-legality)", "(document J3-24-007)", "(clause 7)", "(rule C720)", "(page 80)", "(source-hash " + SOURCE_SHA256 + ")"):
...
def validate_source_binding(doc: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, page_index_path: Path, standardir_path: Path, semantic_input: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c720-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C720", "source rule differs")
    require(source["printed_page"] == 80, "source printed page differs")
...
def validate_fixture_shape(doc: dict[str, Any]) -> None:
    ...
    require({(item["representation_method"], item["computed"]) for item in results} == {("present", "ACCEPTED"), ("absent", "REJECTED"), ("unknown", "UNRESOLVED")}, "fixture does not cover the C720 state table")
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, page_index_path: Path, standardir_path: Path, semantic_input: Path, semantic_output: Path, golden: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "source": {"document": fixture["source"]["document"], "clause": fixture["source"]["clause"], "rule": fixture["source"]["rule"], "printed_page": fixture["source"]["printed_page"], "pdf_sha256": digest(source_pdf), "canonical_text_sha256": digest(canonical_path), "page_index_sha256": digest(page_index_path), "standardir_sha256": digest(standardir_path), "canonical_lines": [item["line"] for item in fixture["source"]["canonical_lines"]], "page_index": fixture["source"]["page_index"], "standardir_rules": [row["rule"] for row in fixture["source"]["standardir"]["rows"]]},
        ...
        "model_calls": 0, "semantic_promotions": 0, "origin": "MECHANICAL", "oracle": "tests/e2e/validate_m3_c720.py",
    }


def self_test() -> None:
    cases = [("present", "ACCEPTED"), ("absent", "REJECTED"), ("unknown", "UNRESOLVED")]
    for representation_method, expected in cases:
        candidate = {"fact": "kind-param-representation-method-use", "source_rule": "C720", "representation_method": representation_method}
        require(c720_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C720 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    if argv[1:] == ["--self-test"]:
        ...
        return 0
    if len(argv) != 9:
        print("usage: validate_m3_c720.py FIXTURE SEMANTIC_OUTPUT STANDARDIR CANONICAL_TEXT PAGE_INDEX PDF GOLDEN RESULT_JSON", file=sys.stderr)
        return 2
    fixture_path, semantic_output, standardir, canonical_text, page_index, source_pdf, golden, result_path = map(Path, argv[1:])
    root = fixture_path.resolve().parents[2]
    try:
        fixture = json.loads(fixture_path.read_text(encoding="utf-8"))
        result = build_result(fixture_path.resolve(), fixture, root, source_pdf.resolve(), canonical_text.resolve(), page_index.resolve(), standardir.resolve(), root / fixture["semantic_item"]["path"], semantic_output.resolve(), golden.resolve())
        result_path.write_text(json.dumps(result, indent=2, sort_keys=True) + "\n", encoding="utf-8")
    except (ContractError, OSError, KeyError, TypeError, ValueError) as error:
        print(f"M3 C720 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C720 validator: PASS")
    return 0
