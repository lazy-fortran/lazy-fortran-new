...
oracle="$ROOT/tests/e2e/oracle_generated_print_expression_decimal_constant.py"
run_dir="$(mktemp -d)"

bash "$ROOT/scripts/check-contracts.sh" >/dev/null
bash "$ROOT/scripts/check_pins.sh" >/dev/null
bash "$ROOT/tests/e2e/check-generated-print-expression-ascii-subtract.sh" >/dev/null
...
run_positive() {
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'decimal-constant ELF mutation was accepted: %s\n' "$source" >&2; exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'decimal-constant negative accepted: %s\n' "$source" >&2; exit 1
    fi
    [ ! -e "$ast" ] || { printf 'decimal-constant negative wrote AST: %s\n' "$source" >&2; exit 1; }
}

run_positive "$ROOT/tests/fixtures/l3-print-expression-decimal-constant-v0.f90" expression-decimal-first
run_positive "$ROOT/tests/fixtures/l3-print-expression-decimal-constant-wide-v0.f90" expression-decimal-second
run_negative "$ROOT/tests/negative/l3-print-expression-decimal-constant-v0-missing-operand.f90" missing-operand
run_negative "$ROOT/tests/negative/l3-print-expression-decimal-constant-v0-real-right.f90" real-right
run_negative "$ROOT/tests/negative/l3-print-expression-decimal-constant-v0-out-of-range.f90" out-of-range
run_negative "$ROOT/tests/negative/l3-print-expression-decimal-constant-v0-write.f90" write
run_negative "$ROOT/tests/negative/l3-print-expression-decimal-constant-v0-wrong-name.f90" wrong-name

printf '%s\n' 'generic decimal-constant-expression PASS'
