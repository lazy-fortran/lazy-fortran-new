#!/usr/bin/env python3
"""Independent validator for the bounded M3 C718 semantic slice."""
...
PROPERTY = "scalar-int-constant-legality"
EXPECTED_CANONICAL_LINES = [
    {"line": 3296, "text": "C718 (R709) A scalar-int-constant-name shall be a named constant of type integer."}
]
...
def c718_oracle(candidate: dict[str, Any]) -> str:
    """Apply C718 to a typed named-constant/type state."""

    named_constant = candidate["named_constant"]
    value_type = candidate["value_type"]
    if named_constant == "unknown" or value_type == "unknown":
        return "UNRESOLVED"
    if named_constant == "present" and value_type == "integer":
        return "ACCEPTED"
    return "REJECTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "named_constant", "value_type"}, f"case {case['id']} candidate")
    require(candidate["fact"] == "scalar-int-constant-use", f"case {case['id']} candidate fact differs")
    require(candidate["source_rule"] == "C718", f"case {case['id']} source rule differs")
    require(candidate["named_constant"] in {"present", "absent", "unknown"}, f"case {case['id']} named-constant state is invalid")
    require(candidate["value_type"] in {"integer", "noninteger", "unknown"}, f"case {case['id']} type state is invalid")
    computed = c718_oracle(candidate)
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(record scalar-int-constant-use" in schema, "contract schema lacks typed scalar-int-constant fact")
    require("(named-constant named-constant-state)" in schema, "contract schema lacks named-constant state")
    require("(type type-state)" in schema, "contract schema lacks type state")
...
def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in (
        "(contract m3-c718-scalar-int-constant-oracle)",
        "(property scalar-int-constant-legality)",
        "(document J3-24-007)",
        "(clause 7)",
        "(rule C718)",
...
def validate_source_binding(
    ...
    require(doc["schema_version"] == "m3-c718-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C718", "source rule differs")
...
def build_result(
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c718.py",
    }


def self_test() -> None:
    cases = [
        ({"fact": "scalar-int-constant-use", "source_rule": "C718", "named_constant": "present", "value_type": "integer"}, "ACCEPTED"),
        ({"fact": "scalar-int-constant-use", "source_rule": "C718", "named_constant": "absent", "value_type": "integer"}, "REJECTED"),
        ({"fact": "scalar-int-constant-use", "source_rule": "C718", "named_constant": "present", "value_type": "noninteger"}, "REJECTED"),
        ({"fact": "scalar-int-constant-use", "source_rule": "C718", "named_constant": "unknown", "value_type": "unknown"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c718_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C718 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 8:
        print(
            "usage: validate_m3_c718.py FIXTURE SEMANTIC_OUTPUT STANDARDIR "
    ...
    try:
        ...
        print(f"M3 C718 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C718 validator: PASS")
    return 0
