...
def main() -> None:
    ...
    if mode == "stop-7":
        ...
        require(source_path is not None, "raw scalar binary source is missing")
        source = source_path.read_text(encoding="utf-8")
        require(source == expected_source, "raw scalar binary source changed")
        ...
        require(re.findall(r"\(storage-key ([^)]+)\)", mir) ==
                ...
                "raw scalar binary MIR storage keys differ")
        require(elf.startswith(b"\x7fELF"), "raw scalar binary artifact is not ELF")
        runtime = subprocess.run(["qemu-riscv64", str(elf_path)], capture_output=True, check=False)
        require(runtime.returncode == 0 and runtime.stdout == expected_output,
                "raw scalar binary runtime differs")
