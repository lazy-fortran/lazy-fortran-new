...
oracle="$ROOT/tests/e2e/oracle_generated_print_variable_positive_initializer.py"
run_dir="$(mktemp -d)"

bash "$ROOT/tests/e2e/check-generated-print-variable-signed-initializer.sh" >/dev/null
...
run_positive() {
    ...
    if python3 "$oracle" "$mutated_ast" "$mir" "$elf" "$source" >/dev/null 2>&1; then
        printf 'positive initializer AST mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
    sed "0,/(literal $value)/s//(literal 1)/" "$mir" > "$mutated_mir"
    if python3 "$oracle" "$ast" "$mutated_mir" "$elf" "$source" >/dev/null 2>&1; then
        printf 'positive initializer MIR mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
    cp "$elf" "$mutated_elf"
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'positive initializer ELF mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'positive initializer negative accepted: %s\n' "$source" >&2
        exit 1
    fi
    [ ! -e "$ast" ] || { printf 'positive initializer negative wrote AST: %s\n' "$source" >&2; exit 1; }
}

run_positive "$ROOT/tests/fixtures/l3-print-variable-positive-initializer-v0.f90" zero
run_positive "$ROOT/tests/fixtures/l3-print-variable-positive-initializer-boundary-v0.f90" boundary
run_negative "$ROOT/tests/negative/l3-print-variable-positive-initializer-out-of-range-v0.f90" out-of-range
run_negative "$ROOT/tests/negative/l3-print-variable-positive-initializer-real-v0.f90" real

printf '%s\n' 'generated print-variable positive initializer PASS'
