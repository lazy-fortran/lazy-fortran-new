...
EVIDENCE_ROOT="${C761_EVIDENCE_ROOT:-/home/ert/code/lazy-fortran-new}"
STANDARD="${STANDARD_NEW_ROOT:-$(cd "$ROOT/../standard-new" 2>/dev/null && pwd)}"
EXPECTED_CENTRAL_COMMIT="${M3_C761_EXPECTED_CENTRAL_COMMIT:-}"
EXPECTED_STANDARD_COMMIT="f94c4c51b51fce22b533b7eeda08741970320913"
RUN_ROOT="$ROOT/.cache/runs/E0228"
if [ "${1:-}" = "--fresh" ]; then
  n=1; while [ -e "$RUN_ROOT/R$(printf '%06d' "$n")" ]; do n=$((n + 1)); done
  RUN_DIR="$RUN_ROOT/R$(printf '%06d' "$n")"
elif [ "$#" = 1 ] && [[ "$1" == .cache/runs/E0228/* ]]; then
  RUN_DIR="$ROOT/$1"
else
  echo "usage: $0 --fresh|.cache/runs/E0228/R<run>" >&2; exit 2
fi
...
(cd "$STANDARD" && fo exec --no-build sxsemantic "$ROOT/tests/fixtures/m3-c761-semantic-items.sx" "$RUN_DIR/semantic-items.canonical.sx") >"$RUN_DIR/sxsemantic.log" 2>&1
cmp "$RUN_DIR/semantic-items.canonical.sx" "$ROOT/tests/fixtures/m3-c761-semantic-items.sx"
C761_EVIDENCE_ROOT="$EVIDENCE_ROOT" python3 "$ROOT/tests/e2e/validate_m3_c761.py" --self-test >"$RUN_DIR/oracle-self-test.log"
C761_EVIDENCE_ROOT="$EVIDENCE_ROOT" python3 "$ROOT/tests/e2e/validate_m3_c761.py" \
  "$ROOT/tests/fixtures/m3-c761-source-backed-v0.json" "$ROOT/tests/fixtures/m3-c761-expected-outcomes-v0.json" \
...
record = {"central": {"repository": "lazy-fortran-new", "commit": central}, "standard_new": {"repository": "standard-new", "commit": standard}, "toolchain": {"python": first(["python3", "--version"]), "git": first(["git", "--version"]), "compiler": first(["gfortran", "--version"]), "poppler": first(["pdftotext", "-v"])}, "environment": {"os": platform.system(), "architecture": platform.machine(), "python_version": platform.python_version()}, "oracle_sha256": hashlib.sha256(Path(validator).read_bytes()).hexdigest(), "commands": ["M3_C761_EXPECTED_CENTRAL_COMMIT=<pinned-central> C761_EVIDENCE_ROOT=<evidence-root> STANDARD_NEW_ROOT=<standard-new-root> tests/e2e/run-m3-c761.sh --fresh", "scripts/check-contracts.sh", "scripts/fetch.sh j3-24-007", "python3 tests/e2e/validate_m3_c761.py --self-test", "python3 tests/e2e/validate_m3_c761.py ..."]}
