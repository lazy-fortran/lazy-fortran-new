#!/usr/bin/env python3
"""Independent validator for the bounded M3 C717 semantic slice."""
...
PROPERTY = "kind-selector-legality"
EXPECTED_CANONICAL_LINES = [
    {"line": 3263, "text": "C717 (R706) The value of scalar-int-constant-expr shall be nonnegative and shall specify a representation"},
    {"line": 3264, "text": "method that exists on the processor."},
]
EXPECTED_STANDARDIR_ROWS = [
    {"rule": "R706", "lhs": "kind-selector", "page": 80, "byte_start": 205461, "byte_length": 64, "occurrence": 56},
]
...
def c717_oracle(candidate: dict[str, Any]) -> str:
    """Apply C717 to abstract value and processor-method states."""
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "kind_value", "representation_method"}, f"case {case['id']} candidate")
    require(candidate["fact"] == "kind-selector-value-use", f"case {case['id']} candidate fact differs")
    require(candidate["source_rule"] == "C717", f"case {case['id']} source rule differs")
    require(candidate["kind_value"] in {"negative", "nonnegative", "unknown"}, f"case {case['id']} kind-value state is invalid")
    require(candidate["representation_method"] in {"absent", "present", "unknown"}, f"case {case['id']} representation-method state is invalid")
    computed = c717_oracle(candidate)
...
def validate_canonical_lines(source: dict[str, Any], canonical_path: Path) -> None:
    ...
    for item in source["canonical_lines"]:
        ...
        require(len(parts) == 2 and parts[0].isdigit(), f"canonical source lacks line marker: {number}")
...
def validate_page_index(source: dict[str, Any], page_index_path: Path) -> None:
    ...
    require(f"page 80 start {expected['start']} length {expected['length']}" in page_index_path.read_text(encoding="utf-8"), "page index span differs")
...
def validate_source_binding(doc: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, page_index_path: Path, standardir_path: Path, semantic_input: Path) -> None:
    ...
    require(source["rule"] == "C717", "source rule differs")
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, page_index_path: Path, standardir_path: Path, semantic_input: Path, semantic_output: Path, golden: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c717.py",
    }
...
def self_test() -> None:
    ...
    print("M3 C717 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    if argv[1:] == ["--self-test"]:
        ...
        return 0
    if len(argv) != 9:
        print("usage: validate_m3_c717.py FIXTURE SEMANTIC_OUTPUT STANDARDIR CANONICAL_TEXT PAGE_INDEX PDF GOLDEN RESULT_JSON", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"M3 C717 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C717 validator: PASS")
    return 0
