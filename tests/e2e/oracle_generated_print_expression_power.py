...
def check_contract(root: pathlib.Path) -> None:
    fixture = root / "contracts/fixtures/l3-print-expression-power-v0.sx"
    text = fixture.read_text(encoding="utf-8")
    for witness in (
        "(contract l3-print-expression-power)", "(version 0)",
        "(origin mechanical)", "(resolution resolved)",
        "(property generic-integer-power-expression-output)",
        "(rules R901 R1006 R1008 R1009 R1212 R1215 R1217)",
        "(pages 155 169 242 244 248)",
    ):
        require(witness in text, f"power contract differs: {witness}")
    cases = {
        "tests/fixtures/l3-print-expression-power-v0.f90":
            "7506174ec7c847968d2f924a0b39f878788a25758a0debdf4470c400afc8e033",
        "tests/fixtures/l3-print-expression-power-wide-v0.f90":
            "762a82612c1aaca325da2b321d6c3740e911d95be302ca5a302e540354d3ed82",
        "tests/negative/l3-print-expression-power-v0-missing-operand.f90":
            "3b7c0a788a5c9bd6a42df32124f069a778f759656491975958615eafee844496",
        "tests/negative/l3-print-expression-power-v0-wrong-value.f90":
            "373a5260c68ca30b7e9e4b55564dd0e63e680835429ad43f183e4bbbe8181644",
        "tests/negative/l3-print-expression-power-v0-write.f90":
            "89d2a84f56a6051ff02d85a2adde18266499c3f64e172ad38a333f0b1d88abde",
        "tests/negative/l3-print-expression-power-v0-wrong-name.f90":
    ...
    for relative, expected in cases.items():
        ...
        require(f"(path {relative})" in text and f"(sha256 {expected})" in text,
                f"power source case differs: {relative}")
        require(digest(source) == expected, f"power source hash differs: {relative}")
    ...
    require(f"(pdf-sha256 {pdf_hash})" in text and digest(pdf) == pdf_hash, "power PDF differs")
    require(f"(standardir-sha256 {ir_hash})" in text and digest(ir) == ir_hash, "power StandardIR differs")


def parse_items(source: pathlib.Path) -> list[tuple[str, str]]:
    match = re.search(r"print \*, (.*)", source.read_text(encoding="utf-8"))
    require(match is not None, "power source has no PRINT list")
    result = []
    for raw in match.group(1).strip().split(","):
        ...
        if value == "x ** 2":
            ...
            raise OracleFailure(f"unsupported power item: {value}")
    return result
...
def check_ast(path: pathlib.Path, source: pathlib.Path, items: list[tuple[str, str]]) -> None:
    ...
    require("(program-unit-v2 " in text and f"(file {source})" in text, "power AST root differs")
    spans = groups(text, "(source-span ")
    require(spans, "power AST has no spans")
    for span in spans:
        require(f"(file {source})" in span and f"(source-hash {identity})" in span,
                "power AST span provenance differs")
    require(f"(source-identity {identity})" in text, "power AST identity differs")
    cursor = text.index("(output-items ")
    require(f"(output-count {len(items)})" in text, "power AST count differs")
    for kind, value in items:
        ...
        require(position >= 0, f"power AST item missing: {witness}")
    ...
    for witness in (
        ...
        require(witness in text, f"power AST provenance missing: {witness}")
...
def check_mir(path: pathlib.Path, items: list[tuple[str, str]]) -> None:
    ...
    for kind, value in items:
        if kind == "expression":
            expected += [("load", "(storage-key x)"), ("const", "(literal 2)"), ("pow", ""), ("output", "")]
        elif kind == "variable":
            expected += [("load", "(storage-key x)"), ("output", "")]
        else:
            expected += [("const", f"(literal {value})"), ("output", "")]
    require(len(printed) == len(expected) + 1, "power MIR count differs")
    for actual, (opcode, operand) in zip(printed, expected):
        require(f"(opcode {opcode})" in actual, f"power MIR opcode differs: {opcode}")
        if operand:
            require(operand in actual, f"power MIR operand differs: {operand}")
    require("(opcode return)" in printed[-1], "power MIR has no return")


def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_expression_power.py AST MIR ELF SOURCE")
    ...
    check_mir(mir, items)
    require(elf.read_bytes().startswith(b"\x7fELF"), "power artifact is not ELF")
    ...
    expected = b"".join((b"9\n" if kind == "expression" else b"3\n" if kind == "variable" else f"{value}\n".encode())
                         for kind, value in items)
    require(runtime.returncode == 0 and runtime.stdout == expected, "power runtime output differs")
    print(f"generic power-expression oracle PASS: {len(items)} items")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic power-expression oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
