...
run_positive() {
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'cardinality ELF mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'cardinality negative accepted: %s\n' "$source" >&2
        exit 1
    fi
    [ ! -e "$ast" ] || { printf 'cardinality negative wrote AST: %s\n' "$source" >&2; exit 1; }
}

run_positive "$ROOT/tests/fixtures/l3-print-list-cardinality-one-v0.f90" one
run_positive "$ROOT/tests/fixtures/l3-print-list-cardinality-four-v0.f90" four
run_positive "$ROOT/tests/fixtures/l3-print-list-cardinality-ten-v0.f90" ten
run_negative "$ROOT/tests/negative/l3-print-list-cardinality-eleven-v0.f90" eleven

printf '%s\n' 'generic print-list-cardinality PASS'
