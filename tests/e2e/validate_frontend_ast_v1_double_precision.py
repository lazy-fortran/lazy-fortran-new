#!/usr/bin/env python3
"""Independent oracle for the bounded source-derived DOUBLE PRECISION replay."""
...
def main() -> int:
    if len(sys.argv) != 4:
        raise ValidationError("usage: validate_frontend_ast_v1_double_precision.py replay-manifest run-dir frontend")
    ...
    contract_path = root / replay["contract_manifest"]
    contract = tomllib.loads(contract_path.read_text(encoding="utf-8"))
    oracle = tomllib.loads((root / contract["expected_oracle"]).read_text(encoding="utf-8"))
    require(replay["boundary"] == "raw-source-to-typed-frontend-ast-v1-source-derived-double-precision-type-spec", "replay boundary differs")
    ...
    require(contract["origin"] == "LLM" and contract["property"] == "source-derived-double-precision-type-spec", "contract identity differs")
    require(contract["model_calls"] == 0 and contract["semantic_promotions"] == 0, "contract guard differs")
    require(oracle["origin"] == "LLM" and oracle["property"] == "source-derived-double-precision-type-spec", "oracle identity differs")
    ...
    for case_id, case in expected.items():
        ...
        expected_source = DOUBLE_SOURCE if case_id == "double-precision" else REAL_SOURCE
        ...
        require(case["expected_root_name"] == "main" and case["expected_program_declaration_name"] == "main", f"identity differs for {case_id}")
        require(case["expected_variable_type"] == expected_types[case_id] and case["expected_variable_name"] == "x", f"expected fields differ for {case_id}")
        ...
        require(oracle_case["expected_root_name"] == "main" and oracle_case["expected_program_declaration_name"] == "main", f"oracle identity differs for {case_id}")
        require(oracle_case["expected_variable_type"] == expected_types[case_id] and oracle_case["expected_variable_name"] == "x", f"oracle fields differ for {case_id}")
        if case_id == "double-precision":
            require(oracle_case["expected_source_type_spec"] == "double precision", "oracle source type spelling differs")
    require(not (run_dir / "negative.ast.sx").exists(), "negative AST was written")
    negative = root / contract["negative"]
    require(negative.read_bytes() == b"program main\n  double precision ::\nend program main\n", "negative source differs")
    ...
    print("frontend AST v1 DOUBLE PRECISION oracle PASS: DOUBLE PRECISION and REAL controls accepted, malformed DOUBLE PRECISION rejected")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"frontend AST v1 DOUBLE PRECISION oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
