#!/usr/bin/env bash
# Run the bounded M3 C744 END TYPE name relation contract.
...
if [ "$#" -eq 1 ] && [ "$1" = "--fresh" ]; then
    run_number=1
    while :; do
        run_arg=".cache/runs/E0206/R$(printf '%06d' "$run_number")"
    ...
    case "$run_arg" in
        .cache/runs/E0206/*) ;;
        *) die "run directory must be below E0206" ;;
    esac
    run_dir="$ROOT/$run_arg"
else
    die "usage: $0 .cache/runs/E0206/R<fresh-run>|--fresh"
fi
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c744-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c744-semantic-items.sx"
...
expected_central_commit="${M3_C744_EXPECTED_CENTRAL_COMMIT:-}"
...
functional_paths=(
    ...
    contracts/fixtures/m3-c744-derived-type-end-type-name-relation-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c744.sh
    tests/e2e/validate_m3_c744.py
    tests/fixtures/m3-c744-semantic-items.sx
    tests/fixtures/m3-c744-source-backed-v0.json
    tests/golden/m3-c744-semantic-items.sx
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || die "functional C744 files differ from manifest central pin $central_pin"
...
record = {
    ...
    "commands": ["M3_C744_EXPECTED_CENTRAL_COMMIT=<pinned-central-revision> tests/e2e/run-m3-c744.sh --fresh", "scripts/check_pins.sh", "scripts/check-contracts.sh", "scripts/check-contracts.sh --self-test", "scripts/fetch.sh j3-24-007", "(cd standard-new && fo clean)", "(cd standard-new && fo)", "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c744-semantic-items.sx <run-dir>/semantic-items.canonical.sx)", "python3 tests/e2e/validate_m3_c744.py --self-test", "python3 tests/e2e/validate_m3_c744.py ..."],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C744 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C744 gate"
printf 'M3 C744 PASS: %s\n' "$run_dir"
