#!/usr/bin/env bash
# Run the bounded M3 C1106 semantic-oracle delivery contract.
...
[ "$#" -eq 1 ] || die "usage: $0 .cache/runs/E0175/R000474"
run_arg="$1"
case "$run_arg" in
    .cache/runs/E0175/*) ;;
    *) die "run directory must be below .cache/runs/E0175" ;;
esac
run_dir="$ROOT/$run_arg"
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c1106-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c1106-semantic-items.sx"
...
record = {
    ...
    "inputs": {
        "fixture": {"path": "tests/fixtures/m3-c1106-source-backed-v0.json", "sha256": digest(fixture)},
        "semantic_items": {"path": "tests/fixtures/m3-c1106-semantic-items.sx", "sha256": digest(semantic_input)},
    ...
    "commands": [
        ...
        "(cd standard-new && fo)",
        "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c1106-semantic-items.sx <run-dir>/semantic-items.canonical.sx)",
        "python3 tests/e2e/validate_m3_c1106.py --self-test",
        "python3 tests/e2e/validate_m3_c1106.py ...",
    ],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || \
    die "standard-new became dirty during the M3 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || \
    die "central checkout became dirty during the M3 gate"

printf 'M3 C1106 PASS: %s\n' "$run_dir"
