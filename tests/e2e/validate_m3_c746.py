#!/usr/bin/env python3
"""Independent validator for the bounded M3 C746 semantic slice."""
...
DEFINITION_NAME_PRESENCES = {"absent", "present", "unknown"}
DECLARED_NAME_RELATIONS = {"member", "not-member", "unknown"}
...
EXPECTED_OUTCOMES_SHA256 = "b755d1d52fee369dda6e57cf26d0ae1c676f233207c8611911342d5f218dd581"
EXPECTED_OUTCOMES_PATH = "tests/fixtures/m3-c746-expected-outcomes-v0.json"
PROPERTY = "derived-type-def-type-parameter-name-membership"
SOURCE_SPAN = {"byte_start": 237401, "byte_length": 171, "page_start": 77, "page_end": 77}
CANONICAL_LINES = [
    {"line": 3764, "text": "C746 (R732) A type-param-name in a type-param-def-stmt in a derived-type-def shall be one of the type-param-"},
    {"line": 3765, "text": "names in the derived-type-stmt of that derived-type-def ."},
]
...
SOURCE_BYTES = (
    b"6 C746 (R732) A type-param-name in a type-param-def-stmt in a derived-type-def shall be one of the type-param-\n"
    b"7 names in the derived-type-stmt of that derived-type-def .\n"
)
CONTRACT = {
    "schema": "contracts/m3-c746-derived-type-type-parameter-name-membership-v0.sxs",
    "fixture": "contracts/fixtures/m3-c746-derived-type-type-parameter-name-membership-v0.sx",
    "version": 0,
}
SEMANTIC_ITEM = {
    "path": "tests/fixtures/m3-c746-semantic-items.sx",
    "sha256": "40ec3df4ac6df3dbafc602ae015dc8e3879753166df604ae288e669b24e6f265",
    "id": "S-C746",
    "subject": PROPERTY,
    "document": "J3-24-007",
    "clause": "7",
...
def validate_expected_outcomes(doc: dict[str, Any], path: Path, case_ids: list[str]) -> dict[str, str]:
    ...
    require(doc["schema_version"] == "m3-c746-expected-outcomes-v0", "expected outcomes schema differs")
    require(doc["origin"] == "HUMAN", "expected outcomes origin differs")
    require(doc["property"] == PROPERTY and doc["source_rule"] == "C746", "expected outcomes identity differs")
...
def oracle(candidate: dict[str, str]) -> str:
    if candidate["context"] != "derived-type-def":
        return "UNRESOLVED"
    if candidate["definition_name_presence"] == "absent":
        return "ACCEPTED"
    if candidate["definition_name_presence"] == "present" and candidate["declared_name_relation"] == "member":
        return "ACCEPTED"
    if candidate["definition_name_presence"] == "present" and candidate["declared_name_relation"] == "not-member":
        return "REJECTED"
    return "UNRESOLVED"
...
def validate_case(case: dict[str, Any], expected_outcomes: dict[str, str]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "definition_name_presence", "declared_name_relation", "context"}, f"case {case['id']} candidate")
    require(candidate["fact"] == PROPERTY, f"case {case['id']} property differs")
    require(candidate["source_rule"] == "C746", f"case {case['id']} source rule differs")
    require(candidate["definition_name_presence"] in DEFINITION_NAME_PRESENCES, f"case {case['id']} definition-name presence invalid")
    require(candidate["declared_name_relation"] in DECLARED_NAME_RELATIONS, f"case {case['id']} declared-name relation invalid")
    ...
    return {
        "id": case["id"],
        "kind": case["kind"],
        "definition_name_presence": candidate["definition_name_presence"],
        "declared_name_relation": candidate["declared_name_relation"],
...
def validate_binding(doc: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c746-source-backed-v0" and doc["origin"] == "HUMAN" and doc["property"] == PROPERTY, "fixture identity differs")
    require(doc["contract"] == CONTRACT, "contract identity differs")
    source = doc["source"]
    require(source["document"] == "J3-24-007" and source["clause"] == "7" and source["rule"] == "C746" and source["printed_page"] == 77, "source identity differs")
    ...
    require(lines[3763] == SOURCE_BYTES.splitlines(keepends=True)[0] and lines[3764] == SOURCE_BYTES.splitlines(keepends=True)[1], "canonical lines differ")
    require(source["source_span"] == SOURCE_SPAN and canonical.read_bytes()[237401:237572] == SOURCE_BYTES, "source span differs")
    ...
    for fragment in ("(id S-C746)", "(subject " + PROPERTY + ")", "(origin human)", "(resolution disputed)"):
        require(fragment in text, f"semantic item lacks {fragment}")
    schema = (root / CONTRACT["schema"]).read_text(encoding="utf-8")
    for fragment in ("(enum c746-definition-name-presence absent present unknown)", "(enum c746-declared-name-relation member not-member unknown)", "(record type-parameter-name-membership"):
        require(fragment in schema, f"contract schema lacks {fragment}")
    witness = (root / CONTRACT["fixture"]).read_text(encoding="utf-8")
    for fragment in ("(contract m3-c746-derived-type-type-parameter-name-membership)", "(property " + PROPERTY + ")", "(rule C746)", "(page 77)", "(byte-start 237401)", "(byte-length 171)", "(source-hash " + SOURCE_SHA256 + ")"):
...
def build_result(fixture_path: Path, expected_path: Path, fixture: dict[str, Any], expected_doc: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic_output: Path, golden: Path, output: Path) -> dict[str, Any]:
    ...
    require({(case["definition_name_presence"], case["declared_name_relation"], case["context"]) for case in cases} == {(definition_name_presence, declared_name_relation, context) for definition_name_presence in DEFINITION_NAME_PRESENCES for declared_name_relation in DECLARED_NAME_RELATIONS for context in CONTEXTS}, "27-state table incomplete")
    ...
    result = {
        ...
        "source": {"document": "J3-24-007", "clause": "7", "rule": "C746", "printed_page": 77, "pdf_sha256": digest(pdf), "canonical_text_sha256": digest(canonical), "page_index_sha256": digest(page_index), "standardir_sha256": digest(standardir), "canonical_lines": [3764, 3765], "source_span": SOURCE_SPAN, "page_index": {"path": ".cache/runs/E0001/R000003/j3-24-007.pages.index", "sha256": PAGE_INDEX_SHA256, "pages": [PAGE]}, "standardir_rules": ["R727", "R732", "R733"]},
        ...
        "model_calls": 0, "semantic_promotions": 0, "origin": "MECHANICAL", "oracle": "tests/e2e/validate_m3_c746.py",
    }
...
def self_test(root: Path) -> None:
    fixture_path = root / "tests/fixtures/m3-c746-source-backed-v0.json"
...
def main() -> int:
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "--self-test":
        self_test(root)
        print("C746 oracle self-test PASS")
        return 0
    if len(sys.argv) != 10:
        raise SystemExit("usage: validate_m3_c746.py fixture expected-outcomes semantic-output standardir canonical page-index pdf golden result")
    ...
    try:
        ...
        print(f"C746 oracle FAIL: {error}", file=sys.stderr)
        return 1
