...
def source_values(path: pathlib.Path) -> tuple[str, str]:
    ...
    initializer, divisor = match.groups()
    require(-100 <= int(initializer) <= 2047, "initializer is outside policy range")
    require(1 <= int(divisor) <= 10, "divisor is outside policy range")
    require("  print *, x" in text, "source PRINT variable differs")
    return initializer, divisor


def check_ast(path: pathlib.Path, source: pathlib.Path, initializer: str, divisor: str) -> None:
    text = path.read_text(encoding="utf-8")
    ...
    require(f"(kind binary-expression) (operator /) (left-operand x) (right-operand {divisor})" in text,
            "AST divisor differs")
...
def check_mir(path: pathlib.Path, initializer: str, divisor: str) -> None:
    text = path.read_text(encoding="utf-8")
    instructions = groups(text, "(instruction ")
    require(len(instructions) == 9, "MIR instruction count differs")
    expected = ["const", "store", "load", "const", "div", "store", "load", "output", "return"]
    ...
    require(f"(literal {divisor})" in instructions[3], "MIR divisor differs")
...
def truncating_division(value: int, divisor: int) -> int:
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_divisor.py AST MIR ELF SOURCE")
    ast, mir, elf, source = map(pathlib.Path, sys.argv[1:])
    initializer, divisor = source_values(source)
    check_ast(ast, source, initializer, divisor)
    check_mir(mir, initializer, divisor)
    ...
    expected = truncating_division(int(initializer), int(divisor))
    require(runtime.stdout == f"{expected}\n".encode(), "runtime output differs")
    print("generic initialized divisor oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic initialized divisor oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
