...
EXPECTED_CENTRAL_COMMIT="${M3_C757_EXPECTED_CENTRAL_COMMIT:-}"
...
python3 - "$RUN_DIR/run-environment.json" "$central_commit" "$standard_commit" "$fo_path" "$fo_version" "$fo_sha256" "$ROOT/tests/e2e/validate_m3_c757.py" <<'PY'
import hashlib
import json
import platform
import subprocess
...
record = {
    ...
    "commands": ["M3_C757_EXPECTED_CENTRAL_COMMIT=<pinned-central> C757_EVIDENCE_ROOT=<evidence-root> STANDARD_NEW_ROOT=<standard-new-root> tests/e2e/run-m3-c757.sh --fresh", "scripts/check_pins.sh", "scripts/check-contracts.sh", "scripts/check-contracts.sh --self-test", "scripts/fetch.sh j3-24-007", "python3 tests/e2e/validate_m3_c757.py --self-test", "python3 tests/e2e/validate_m3_c757.py ..."],
}
