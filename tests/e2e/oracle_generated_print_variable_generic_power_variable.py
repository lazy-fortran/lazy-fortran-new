"""Independent oracle for bounded initialized x ** x transport."""
...
def initializer(path: pathlib.Path) -> int:
    ...
    require(2 <= value <= 10, "initialized value is outside variable-power policy")
...
def check_mir(path: pathlib.Path) -> None:
    ...
    expected = ["const", "store", "load", "load", "pow", "store", "load", "output", "return"]
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_power_variable.py AST MIR ELF SOURCE")
    ...
    print("generic initialized variable power oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic initialized variable power oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
