...
def check_contract(root: pathlib.Path) -> None:
    fixture = root / "contracts/fixtures/l3-print-expression-subtract-v0.sx"
    text = fixture.read_text(encoding="utf-8")
    for witness in (
        "(contract l3-print-expression-subtract)", "(version 0)",
        "(origin mechanical)", "(resolution resolved)",
        "(property generic-integer-subtraction-expression-output)",
    ...
    cases = {
        ...
        "tests/negative/l3-print-expression-subtract-v0-ascii-hyphen.f90": "8c12ccfcbb31820eac9edd41a2e8d64a3ee734110d28c2053129db167ec0f4f6",
    }
    for relative, expected in cases.items():
        source = root / relative
        require(f"(path {relative})" in text and f"(sha256 {expected})" in text,
                f"subtract source case differs: {relative}")
        require(digest(source) == expected, f"subtract source hash differs: {relative}")
    ...
    require(f"(pdf-sha256 {pdf_hash})" in text and digest(pdf) == pdf_hash, "subtract PDF differs")
    require(f"(standardir-sha256 {ir_hash})" in text and digest(ir) == ir_hash, "subtract StandardIR differs")


def parse_items(source: pathlib.Path) -> list[tuple[str, str]]:
    match = re.search(r"print \*, (.*)", source.read_text(encoding="utf-8"))
    require(match is not None, "subtract source has no PRINT list")
    result = []
    for raw in match.group(1).strip().split(","):
        ...
        if value == "x – 2":
            ...
            raise OracleFailure(f"unsupported subtract item: {value}")
    return result
...
def check_ast(path: pathlib.Path, source: pathlib.Path, items: list[tuple[str, str]]) -> None:
    ...
    require("(program-unit-v2 " in text and f"(file {source})" in text, "subtract AST root differs")
    spans = groups(text, "(source-span ")
    require(spans, "subtract AST has no spans")
    for span in spans:
        require(f"(file {source})" in span and f"(source-hash {identity})" in span,
                "subtract AST span provenance differs")
    require(f"(source-identity {identity})" in text, "subtract AST identity differs")
    cursor = text.index("(output-items ")
    require(f"(output-count {len(items)})" in text, "subtract AST count differs")
    for kind, value in items:
        ...
        require(position >= 0, f"subtract AST item missing: {witness}")
    ...
    for witness in (
        ...
        require(witness in text, f"subtract AST provenance missing: {witness}")
...
def check_mir(path: pathlib.Path, items: list[tuple[str, str]]) -> None:
    ...
    require(len(printed) == len(expected) + 1, "subtract MIR count differs")
    for actual, (opcode, operand) in zip(printed, expected):
        require(f"(opcode {opcode})" in actual, f"subtract MIR opcode differs: {opcode}")
        if operand:
            require(operand in actual, f"subtract MIR operand differs: {operand}")
    require("(opcode return)" in printed[-1], "subtract MIR has no return")


def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_expression_subtract.py AST MIR ELF SOURCE")
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic subtract-expression oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
