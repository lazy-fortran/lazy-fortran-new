#!/usr/bin/env python3
"""Independent oracle for the bounded typed-AST v1 alpha-name slice."""
...
def main() -> int:
    if len(sys.argv) != 4:
        raise ValidationError(
            "usage: validate_frontend_ast_v1_name_multichar.py manifest run-dir frontend"
        )
    ...
    require(normalized(run_dir / "control.ast.sx") == control_expected,
            "changed-name control AST differs from established z golden")
    require(not (run_dir / "negative.ast.sx").exists(), "negative AST was written")
    print("frontend AST v1 multichar oracle PASS: alpha and z control executed, malformed neighbour rejected")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"frontend AST v1 multichar oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
