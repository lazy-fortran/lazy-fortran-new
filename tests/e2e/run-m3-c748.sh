...
if [ "$#" -eq 1 ] && [ "$1" = "--fresh" ]; then
    run_number=1
    while :; do
        run_arg=".cache/runs/E0214/R$(printf '%06d' "$run_number")"
    ...
    case "$run_arg" in
        .cache/runs/E0214/*) ;;
        *) die "run directory must be below E0214" ;;
    esac
    run_dir="$ROOT/$run_arg"
else
    die "usage: $0 .cache/runs/E0214/R<fresh-run>|--fresh"
fi
...
expected_central_commit="${M3_C748_EXPECTED_CENTRAL_COMMIT:-}"
...
[ -f "$trace" ] || die "missing committed C748 trace"
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C748 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C748 gate"
printf 'M3 C748 PASS: %s\n' "$run_dir"
