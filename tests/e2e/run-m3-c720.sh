#!/usr/bin/env bash
# Run the bounded M3 C720 kind-param representation-method delivery contract.
...
if [ "$#" -eq 1 ] && [ "$1" = "--fresh" ]; then
    run_number=1
    while :; do
        run_arg=".cache/runs/E0190/R$(printf '%06d' "$run_number")"
    ...
    case "$run_arg" in
        .cache/runs/E0190/*) ;;
        *) die "run directory must be below .cache/runs/E0190" ;;
    esac
    run_dir="$ROOT/$run_arg"
else
    die "usage: $0 .cache/runs/E0190/R<fresh-run>|--fresh"
fi
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c720-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c720-semantic-items.sx"
...
golden="$ROOT/tests/golden/m3-c720-semantic-items.sx"
validator="$ROOT/tests/e2e/validate_m3_c720.py"
manifest="$ROOT/research/experiments/E0190-can-a-deterministic-source-backed-oracle/manifest.yaml"
...
functional_paths=(
    contracts/m3-c720-kind-param-representation-oracle-v0.sxs
    contracts/fixtures/m3-c720-kind-param-representation-oracle-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c720.sh
    tests/e2e/validate_m3_c720.py
    tests/fixtures/m3-c720-semantic-items.sx
    tests/fixtures/m3-c720-source-backed-v0.json
    tests/golden/m3-c720-semantic-items.sx
    artifacts/traces/m3-c720-source-backed-v0.json
    research/decisions/D0141-fifteenth-m3-c720-kind-param-representation-oracle.md
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || die "functional M3 C720 files differ from manifest central pin $central_pin"
...
record = {
    ...
    "inputs": {
        ...
        "semantic_items": {"path": "tests/fixtures/m3-c720-semantic-items.sx", "sha256": digest(semantic_input)},
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C720 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C720 gate"
printf 'M3 C720 PASS: %s\n' "$run_dir"
