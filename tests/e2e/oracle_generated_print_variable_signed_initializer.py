#!/usr/bin/env python3
"""Independent oracle for bounded signed variable initialization and PRINT."""
...
def source_value(path: pathlib.Path) -> str:
    ...
    value = match.group(1)
    require(-100 <= int(value) <= -1, "source initializer is out of bounded range")
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_signed_initializer.py AST MIR ELF SOURCE")
    ...
    print("signed variable initializer oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"signed variable initializer oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
