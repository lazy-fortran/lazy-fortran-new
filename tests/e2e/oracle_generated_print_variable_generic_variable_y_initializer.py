#!/usr/bin/env python3
"""Independent oracle for generated in-range variable-y initializer transport."""
...
def check_ast(path: pathlib.Path, source: pathlib.Path, value: str) -> None:
    ...
    require("(output-kind variable) (output-name y)" in text, "AST PRINT variable y differs")
    for witness in (
...
def check_mir(path: pathlib.Path, value: str) -> None:
    ...
    expected = ["const", "store", "load", "output", "return"]
    ...
    require("(storage-key y)" in instructions[1], "MIR store key differs")
    require("(storage-key y)" in instructions[2], "MIR load key differs")
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_variable_y_initializer.py AST MIR ELF SOURCE")
    ...
    print("generic variable-y initializer oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic variable-y initializer oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
