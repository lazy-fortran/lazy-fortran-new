#!/usr/bin/env python3
"""Independent validator for the bounded M3 C724 semantic slice."""
...
VALUE_SIGNS = {"negative", "nonnegative", "unknown"}
METHOD_STATES = {"absent", "present", "unknown"}
...
PROPERTY = "scalar-int-constant-expr-legality"
...
def c724_oracle(candidate: dict[str, Any]) -> str:
    """Apply only C724's typed known-violation relation."""

    if candidate["value_sign"] == "negative" or candidate["representation_method"] == "absent":
        return "REJECTED"
    if candidate["value_sign"] == "nonnegative" and candidate["representation_method"] == "present":
        return "ACCEPTED"
    return "UNRESOLVED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "value_sign", "representation_method"}, f"case {case['id']} candidate")
    require(candidate["fact"] == PROPERTY, f"case {case['id']} candidate fact differs")
    require(candidate["source_rule"] == "C724", f"case {case['id']} source rule differs")
    require(candidate["value_sign"] in VALUE_SIGNS, f"case {case['id']} value-sign state is invalid")
    require(candidate["representation_method"] in METHOD_STATES, f"case {case['id']} method state is invalid")
    computed = c724_oracle(candidate)
    require(computed == case["expected"], f"case {case['id']} expected outcome disagrees with oracle")
    return {
        "id": case["id"],
        "kind": case["kind"],
        "value_sign": candidate["value_sign"],
...
def validate_standardir(source: dict[str, Any], path: Path) -> None:
    ...
    expected = [{"rule": "R721", "lhs": "char-selector", "page": 84, "byte_start": 217200, "byte_length": 251, "occurrence": 71}]
...
def validate_source_lines(source: dict[str, Any], path: Path) -> None:
    expected = [
        {"line": 3450, "text": "C724 (R721) The value of scalar-int-constant-expr shall be nonnegative and shall specify a representation"},
        {"line": 3451, "text": "method that exists on the processor."},
    ]
...
def validate_page_index(source: dict[str, Any], path: Path) -> None:
    expected = {"path": ".cache/runs/E0001/R000003/j3-24-007.pages.index", "sha256": PAGE_INDEX_SHA256, "page": 83, "start": 212364, "length": 2632}
    ...
    require("page 83 start 212364 length 2632" in path.read_text(encoding="utf-8"), "page index span differs")
...
def validate_contract(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in ("(enum outcome accepted rejected unresolved)", "(enum value-sign-state negative nonnegative unknown)", "(enum representation-method-state absent present unknown)", "(record scalar-int-constant-expr-legality", "(record semantic-candidate"):
        require(fragment in schema, f"contract schema lacks {fragment}")
    witness = (root / doc["contract"]["fixture"]).read_text(encoding="utf-8")
    for fragment in ("(contract m3-c724-scalar-int-constant-expr-oracle)", "(property scalar-int-constant-expr-legality)", "(document J3-24-007)", "(clause 7)", "(rule C724)", "(page 83)", "(source-hash " + SOURCE_SHA256 + ")"):
...
def validate_source_binding(doc: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c724-source-backed-v0", "fixture schema version differs")
    ...
    source = doc["source"]
    require(source["document"] == "J3-24-007" and source["clause"] == "7" and source["rule"] == "C724" and source["printed_page"] == 83, "source identity differs")
...
def validate_shape(doc: dict[str, Any]) -> None:
    ...
    require({(item["value_sign"], item["representation_method"]) for item in results} == {(value, method) for value in VALUE_SIGNS for method in METHOD_STATES}, "C724 nine-state table is incomplete")
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path, output: Path, golden: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "source": {"document": fixture["source"]["document"], "clause": fixture["source"]["clause"], "rule": fixture["source"]["rule"], "printed_page": fixture["source"]["printed_page"], "pdf_sha256": digest(pdf), "canonical_text_sha256": digest(canonical), "page_index_sha256": digest(page_index), "standardir_sha256": digest(standardir), "canonical_lines": [item["line"] for item in fixture["source"]["canonical_lines"]], "page_index": fixture["source"]["page_index"], "standardir_rules": [row["rule"] for row in fixture["source"]["standardir"]["rows"]]},
        "semantic_items": {"input": fixture["semantic_item"]["path"], "input_sha256": digest(semantic), "canonical_output_sha256": digest(output), "canonical_output": "PASS"},
        "cases": cases, "outcome_counts": counts, "mutation_controls": mutations,
        "model_calls": 0, "semantic_promotions": 0, "origin": "MECHANICAL", "oracle": "tests/e2e/validate_m3_c724.py",
    }


def self_test() -> None:
    expected = {
        ("negative", "negative"): "REJECTED", ("negative", "nonnegative"): "REJECTED", ("negative", "unknown"): "REJECTED",
        ("nonnegative", "absent"): "REJECTED", ("nonnegative", "present"): "ACCEPTED", ("nonnegative", "unknown"): "UNRESOLVED",
        ("unknown", "absent"): "REJECTED", ("unknown", "present"): "UNRESOLVED", ("unknown", "unknown"): "UNRESOLVED",
    }
    for (value_sign, representation_method), outcome in expected.items():
        require(c724_oracle({"value_sign": value_sign, "representation_method": representation_method}) == outcome, f"self-test outcome differs: {value_sign}/{representation_method}")
    print("M3 C724 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    if argv[1:] == ["--self-test"]:
        ...
        return 0
    if len(argv) != 9:
        print("usage: validate_m3_c724.py FIXTURE SEMANTIC_OUTPUT STANDARDIR CANONICAL_TEXT PAGE_INDEX PDF GOLDEN RESULT_JSON", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"M3 C724 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C724 validator: PASS")
    return 0
