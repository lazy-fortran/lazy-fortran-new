#!/usr/bin/env bash
# Run the bounded M3 C731 character-length constant-expression contract.
...
if [ "$#" -eq 1 ] && [ "$1" = "--fresh" ]; then
    run_number=1
    while :; do
        run_arg=".cache/runs/E0196/R$(printf '%06d' "$run_number")"
    ...
    case "$run_arg" in
        .cache/runs/E0196/*) ;;
        *) die "run directory must be below .cache/runs/E0196" ;;
    esac
    run_dir="$ROOT/$run_arg"
else
    die "usage: $0 .cache/runs/E0196/R<fresh-run>|--fresh"
fi
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c731-source-backed-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c731-semantic-items.sx"
...
golden="$ROOT/tests/golden/m3-c731-semantic-items.sx"
validator="$ROOT/tests/e2e/validate_m3_c731.py"
manifest="$ROOT/research/experiments/E0196-can-a-deterministic-source-backed-c731-oracle/manifest.yaml"
standard="$(resolve_repo standard-new)"

central_commit="$(git -C "$ROOT" rev-parse HEAD)"
standard_commit="$(git -C "$standard" rev-parse HEAD)"
expected_central_commit="${M3_C731_EXPECTED_CENTRAL_COMMIT:-}"
...
functional_paths=(
    contracts/m3-c731-constant-expression-oracle-v0.sxs
    contracts/fixtures/m3-c731-constant-expression-oracle-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c731.sh
    tests/e2e/validate_m3_c731.py
    tests/fixtures/m3-c731-semantic-items.sx
    tests/fixtures/m3-c731-source-backed-v0.json
    tests/golden/m3-c731-semantic-items.sx
    artifacts/traces/m3-c731-source-backed-v0.json
    research/decisions/D0145-nineteenth-m3-c731-constant-expression-oracle.md
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || die "functional M3 C731 files differ from manifest central pin $central_pin"
...
[ -f "$ROOT/artifacts/traces/m3-c731-source-backed-v0.json" ] || die "missing committed C731 trace"
cmp "$run_dir/result.json" "$ROOT/artifacts/traces/m3-c731-source-backed-v0.json"
...
record = {
    ...
    "inputs": {
        "fixture": {"path": "tests/fixtures/m3-c731-source-backed-v0.json", "sha256": digest(fixture)},
        "semantic_items": {"path": "tests/fixtures/m3-c731-semantic-items.sx", "sha256": digest(semantic_input)},
    ...
    "commands": ["M3_C731_EXPECTED_CENTRAL_COMMIT=<pinned-central-revision> tests/e2e/run-m3-c731.sh --fresh", "scripts/check_pins.sh", "scripts/check-contracts.sh", "scripts/check-contracts.sh --self-test", "scripts/fetch.sh j3-24-007", "(cd standard-new && fo clean)", "(cd standard-new && fo)", "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c731-semantic-items.sx <run-dir>/semantic-items.canonical.sx)", "python3 tests/e2e/validate_m3_c731.py --self-test", "python3 tests/e2e/validate_m3_c731.py ..."],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C731 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C731 gate"
printf 'M3 C731 PASS: %s\n' "$run_dir"
