...
def source_values(path: pathlib.Path) -> tuple[str, str]:
    ...
    initializer, subtrahend = match.groups()
    require(-100 <= int(initializer) <= 2047, "initializer is outside policy range")
    require(1 <= int(subtrahend) <= 10, "subtrahend is outside policy range")
    require("  print *, x" in text, "source PRINT variable differs")
    return initializer, subtrahend


def check_ast(path: pathlib.Path, source: pathlib.Path, initializer: str, subtrahend: str) -> None:
    text = path.read_text(encoding="utf-8")
    ...
    require(f"(kind binary-expression) (operator -) (left-operand x) (right-operand {subtrahend})" in text,
            "AST subtrahend differs")
...
def check_mir(path: pathlib.Path, initializer: str, subtrahend: str) -> None:
    text = path.read_text(encoding="utf-8")
    instructions = groups(text, "(instruction ")
    require(len(instructions) == 9, "MIR instruction count differs")
    expected = ["const", "store", "load", "const", "sub", "store", "load", "output", "return"]
    ...
    require(f"(literal {subtrahend})" in instructions[3], "MIR subtrahend differs")
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_subtrahend.py AST MIR ELF SOURCE")
    ast, mir, elf, source = map(pathlib.Path, sys.argv[1:])
    initializer, subtrahend = source_values(source)
    check_ast(ast, source, initializer, subtrahend)
    check_mir(mir, initializer, subtrahend)
    ...
    require(runtime.stdout == f"{int(initializer) - int(subtrahend)}\n".encode(),
            "runtime output differs")
    print("generic initialized subtrahend oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic initialized subtrahend oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
