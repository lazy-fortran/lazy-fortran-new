#!/usr/bin/env bash
# Run the bounded M3 C746 type-parameter-name membership contract.
...
if [ "$#" -eq 1 ] && [ "$1" = "--fresh" ]; then
    run_number=1
    while :; do
        run_arg=".cache/runs/E0210/R$(printf '%06d' "$run_number")"
    ...
    case "$run_arg" in
        .cache/runs/E0210/*) ;;
        *) die "run directory must be below E0210" ;;
    esac
    run_dir="$ROOT/$run_arg"
else
    die "usage: $0 .cache/runs/E0210/R<fresh-run>|--fresh"
fi
[ ! -e "$run_dir" ] || die "run directory already exists: $run_dir"
mkdir -p -- "$run_dir"

fixture="$ROOT/tests/fixtures/m3-c746-source-backed-v0.json"
expected_outcomes="$ROOT/tests/fixtures/m3-c746-expected-outcomes-v0.json"
semantic_input="$ROOT/tests/fixtures/m3-c746-semantic-items.sx"
...
golden="$ROOT/tests/golden/m3-c746-semantic-items.sx"
trace="$ROOT/artifacts/traces/m3-c746-source-backed-v0.json"
validator="$ROOT/tests/e2e/validate_m3_c746.py"
manifest="$ROOT/research/experiments/E0210-can-a-deterministic-source-backed-c746-o/manifest.yaml"
standard="$(resolve_repo standard-new)"

central_commit="$(git -C "$ROOT" rev-parse HEAD)"
standard_commit="$(git -C "$standard" rev-parse HEAD)"
expected_central_commit="${M3_C746_EXPECTED_CENTRAL_COMMIT:-}"
...
functional_paths=(
    ...
    contracts/fixtures/m3-c746-derived-type-type-parameter-name-membership-v0.sx
    contracts/registry.sx
    tests/e2e/run-m3-c746.sh
    tests/e2e/validate_m3_c746.py
    tests/fixtures/m3-c746-expected-outcomes-v0.json
    tests/fixtures/m3-c746-semantic-items.sx
    tests/fixtures/m3-c746-source-backed-v0.json
    tests/golden/m3-c746-semantic-items.sx
)
git -C "$ROOT" diff --quiet "$central_pin" -- "${functional_paths[@]}" || die "functional C746 files differ from manifest central pin $central_pin"
...
[ -f "$trace" ] || die "missing committed C746 trace"
...
record = {
    ...
    "inputs": {"fixture": {"path": "tests/fixtures/m3-c746-source-backed-v0.json", "sha256": digest(fixture)}, "expected_outcomes": {"path": "tests/fixtures/m3-c746-expected-outcomes-v0.json", "sha256": digest(expected_outcomes)}, "semantic_items": {"path": "tests/fixtures/m3-c746-semantic-items.sx", "sha256": digest(semantic_input)}, "standardir": {"path": ".cache/runs/E0171/R000433-provenance-replay/standardir.sx", "sha256": digest(standardir)}, "canonical_text": {"path": ".cache/runs/E0001/R000003/j3-24-007.canonical.txt", "sha256": digest(canonical_text)}, "page_index": {"path": ".cache/runs/E0001/R000003/j3-24-007.pages.index", "sha256": digest(page_index)}, "normative_pdf": {"path": ".cache/j3-24-007.pdf", "sha256": digest(source_pdf)}},
    ...
    "commands": ["M3_C746_EXPECTED_CENTRAL_COMMIT=<pinned-central-revision> tests/e2e/run-m3-c746.sh --fresh", "scripts/check_pins.sh", "scripts/check-contracts.sh", "scripts/check-contracts.sh --self-test", "scripts/fetch.sh j3-24-007", "(cd standard-new && fo clean)", "(cd standard-new && fo)", "(cd standard-new && fo exec --no-build sxsemantic tests/fixtures/m3-c746-semantic-items.sx <run-dir>/semantic-items.canonical.sx)", "python3 tests/e2e/validate_m3_c746.py --self-test", "python3 tests/e2e/validate_m3_c746.py ..."],
    "origin": "MECHANICAL",
}
...
[ -z "$(git -C "$standard" status --porcelain --untracked-files=normal)" ] || die "standard-new became dirty during the M3 C746 gate"
[ -z "$(git -C "$ROOT" status --porcelain --untracked-files=normal)" ] || die "central checkout became dirty during the M3 C746 gate"
printf 'M3 C746 PASS: %s\n' "$run_dir"
