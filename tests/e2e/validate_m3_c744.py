#!/usr/bin/env python3
"""Independent validator for the bounded M3 C744 semantic slice."""
...
PRESENCES = {"absent", "present", "unknown"}
RELATIONS = {"same", "different", "unknown"}
...
PROPERTY = "derived-type-def-end-type-name-relation"
SOURCE_SPAN = {"byte_start": 230888, "byte_length": 137, "page_start": 89, "page_end": 89}
CANONICAL_LINES = [
    {"line": 3639, "text": "C744 (R730) If END TYPE is followed by a type-name, the type-name shall be the same as that in the"},
    {"line": 3640, "text": "corresponding derived-type-stmt."},
]
...
STANDARDIR_ROWS = [
    ...
    {"rule": "R730", "lhs": "end-type-stmt", "page": 89, "byte_start": 230841, "byte_length": 46, "occurrence": 80},
]
SOURCE_BYTES = b"9 C744 (R730) If END TYPE is followed by a type-name, the type-name shall be the same as that in the\n10 corresponding derived-type-stmt.\n"
...
def oracle(candidate: dict[str, str]) -> str:
    ...
    if candidate["presence"] == "absent":
        return "ACCEPTED"
    if candidate["presence"] == "present" and candidate["relation"] == "same":
        return "ACCEPTED"
    if candidate["presence"] == "present" and candidate["relation"] == "different":
        return "REJECTED"
    return "UNRESOLVED"
...
def validate_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "presence", "relation", "context"}, f"case {case['id']} candidate")
    require(candidate["fact"] == PROPERTY, f"case {case['id']} property differs")
    require(candidate["source_rule"] == "C744", f"case {case['id']} source rule differs")
    require(candidate["presence"] in PRESENCES, f"case {case['id']} presence invalid")
    require(candidate["relation"] in RELATIONS, f"case {case['id']} relation invalid")
    ...
    return {"id": case["id"], "kind": case["kind"], "presence": candidate["presence"], "relation": candidate["relation"], "context": candidate["context"], "computed": computed, "expected": case["expected"]}
...
def validate_standardir(source: dict[str, Any], path: Path) -> None:
    ...
    r730 = next(line for line in lines if line.startswith("(syntax R730 "))
    require("(lhs derived-type-stmt)" in r727, "R727 shape differs")
    require("(lhs end-type-stmt)" in r730 and "(optional (ref type-name))" in r730, "R730 shape differs")
...
def validate_binding(doc: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c744-source-backed-v0" and doc["origin"] == "HUMAN" and doc["property"] == PROPERTY, "fixture identity differs")
    source = doc["source"]
    require(source["document"] == "J3-24-007" and source["clause"] == "7" and source["rule"] == "C744" and source["printed_page"] == 89, "source identity differs")
    ...
    require(lines[3638] == SOURCE_BYTES.splitlines(keepends=True)[0] and lines[3639] == SOURCE_BYTES.splitlines(keepends=True)[1], "canonical lines differ")
    require(source["source_span"] == SOURCE_SPAN and canonical.read_bytes()[230888:231025] == SOURCE_BYTES, "source span differs")
    ...
    require(item["document"] == "J3-24-007" and item["clause"] == "7" and item["rule"] == "C744" and item["page"] == 89 and item["source_hash"] == SOURCE_SHA256, "semantic source identity differs")
    ...
    for fragment in ("(id S-C744)", "(subject " + PROPERTY + ")", "(origin human)", "(resolution disputed)"):
        require(fragment in text, f"semantic item lacks {fragment}")
    schema = (root / doc["contract"]["schema"]).read_text(encoding="utf-8")
    for fragment in ("(enum c744-name-presence absent present unknown)", "(enum c744-name-relation same different unknown)", "(record end-type-name-use"):
        require(fragment in schema, f"contract schema lacks {fragment}")
    witness = (root / doc["contract"]["fixture"]).read_text(encoding="utf-8")
    for fragment in ("(contract m3-c744-derived-type-end-type-name-relation)", "(property " + PROPERTY + ")", "(rule C744)", "(page 89)", "(byte-start 230888)", "(byte-length 137)", "(source-hash " + SOURCE_SHA256 + ")"):
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path, semantic_output: Path, golden: Path, output: Path) -> dict[str, Any]:
    ...
    require({(case["presence"], case["relation"], case["context"]) for case in cases} == {(presence, relation, context) for presence in PRESENCES for relation in RELATIONS for context in CONTEXTS}, "27-state table incomplete")
    ...
    result = {
        ...
        "source": {"document": "J3-24-007", "clause": "7", "rule": "C744", "printed_page": 89, "pdf_sha256": digest(pdf), "canonical_text_sha256": digest(canonical), "page_index_sha256": digest(page_index), "standardir_sha256": digest(standardir), "canonical_lines": [3639, 3640], "source_span": SOURCE_SPAN, "page_index": {"path": ".cache/runs/E0001/R000003/j3-24-007.pages.index", "sha256": PAGE_INDEX_SHA256, "pages": [PAGE]}, "standardir_rules": ["R727", "R730"]},
        "semantic_items": {"input": "tests/fixtures/m3-c744-semantic-items.sx", "input_sha256": digest(semantic), "canonical_output_sha256": digest(semantic_output), "canonical_output": "PASS"},
        ...
        "model_calls": 0, "semantic_promotions": 0, "origin": "MECHANICAL", "oracle": "tests/e2e/validate_m3_c744.py",
    }
    ...
    for presence in PRESENCES:
        for relation in RELATIONS:
            for context in CONTEXTS:
                expected = "UNRESOLVED"
                if context == "derived-type-def" and presence == "absent":
                    expected = "ACCEPTED"
                elif context == "derived-type-def" and presence == "present" and relation == "same":
                    expected = "ACCEPTED"
                elif context == "derived-type-def" and presence == "present" and relation == "different":
                    expected = "REJECTED"
                require(oracle({"presence": presence, "relation": relation, "context": context}) == expected, f"self-test failed for {presence}/{relation}/{context}")


def main() -> int:
    if len(sys.argv) == 2 and sys.argv[1] == "--self-test":
        self_test()
        print("C744 oracle self-test PASS")
        return 0
    if len(sys.argv) != 9:
        raise SystemExit("usage: validate_m3_c744.py fixture semantic-output standardir canonical page-index pdf golden result")
    fixture_path, semantic_output, standardir, canonical, page_index, pdf, golden, output = map(Path, sys.argv[1:])
    root = Path(__file__).resolve().parents[2]
    try:
        fixture = json.loads(fixture_path.read_text(encoding="utf-8"))
        result = build_result(fixture_path, fixture, root, pdf, canonical, page_index, standardir, root / "tests/fixtures/m3-c744-semantic-items.sx", semantic_output, golden, output)
    except (ContractError, OSError, KeyError, TypeError, ValueError) as error:
        print(f"C744 oracle FAIL: {error}", file=sys.stderr)
        return 1
