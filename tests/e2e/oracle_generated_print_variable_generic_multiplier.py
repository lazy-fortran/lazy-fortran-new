...
def source_values(path: pathlib.Path) -> tuple[str, str]:
    ...
    initializer, multiplier = match.groups()
    require(-100 <= int(initializer) <= 2047, "initializer is outside policy range")
    require(1 <= int(multiplier) <= 10, "multiplier is outside policy range")
    require("  print *, x" in text, "source PRINT variable differs")
    return initializer, multiplier


def check_ast(path: pathlib.Path, source: pathlib.Path, initializer: str, multiplier: str) -> None:
    text = path.read_text(encoding="utf-8")
    ...
    require(f"(kind binary-expression) (operator *) (left-operand x) (right-operand {multiplier})" in text,
            "AST multiplier differs")
...
def check_mir(path: pathlib.Path, initializer: str, multiplier: str) -> None:
    text = path.read_text(encoding="utf-8")
    instructions = groups(text, "(instruction ")
    require(len(instructions) == 9, "MIR instruction count differs")
    expected = ["const", "store", "load", "const", "mul", "store", "load", "output", "return"]
    ...
    require(f"(literal {multiplier})" in instructions[3], "MIR multiplier differs")
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_multiplier.py AST MIR ELF SOURCE")
    ast, mir, elf, source = map(pathlib.Path, sys.argv[1:])
    initializer, multiplier = source_values(source)
    check_ast(ast, source, initializer, multiplier)
    check_mir(mir, initializer, multiplier)
    ...
    require(runtime.stdout == f"{int(initializer) * int(multiplier)}\n".encode(),
            "runtime output differs")
    print("generic initialized multiplier oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic initialized multiplier oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
