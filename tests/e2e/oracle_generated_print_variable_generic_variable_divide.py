...
def initializer(path: pathlib.Path) -> int:
    ...
    require(-100 <= value <= 2047 and value != 0, "initialized value is outside integer policy")
...
def check_mir(path: pathlib.Path) -> None:
    ...
    expected = ["const", "store", "load", "load", "div", "store", "load", "output", "return"]
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_variable_divide.py AST MIR ELF SOURCE")
    ...
    require(runtime.stdout == f"{int(value / value)}\n".encode(), "runtime output differs")
    print("generic initialized variable divide oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic initialized variable divide oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
