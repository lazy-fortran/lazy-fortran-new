#!/usr/bin/env python3
"""Independent validator for the bounded M3 C735 semantic slice."""
...
OCCURRENCES = {"none", "distinct", "duplicate", "unknown"}
CONTEXTS = {"derived-type-stmt", "other", "unknown"}
...
PROPERTY = "derived-type-stmt-type-attr-uniqueness"
SOURCE_SPAN = {"byte_start": 229534, "byte_length": 101, "page_start": 88, "page_end": 88}
CANONICAL_LINE = {"line": 3620, "text": "C735 (R727) The same type-attr-spec shall not appear more than once in a given derived-type-stmt."}
PAGE = {"page": 88, "start": 228013, "length": 2274}
STANDARDIR_ROWS = [
...
SOURCE_BYTES = b"22 C735 (R727) The same type-attr-spec shall not appear more than once in a given derived-type-stmt.\n"
...
def oracle(candidate: dict[str, str]) -> str:
    if candidate["context"] != "derived-type-stmt":
        return "UNRESOLVED"
    if candidate["attr_occurrence"] in {"none", "distinct"}:
        return "ACCEPTED"
    if candidate["attr_occurrence"] == "duplicate":
        return "REJECTED"
    return "UNRESOLVED"
...
def validate_case(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"fact", "source_rule", "attr_occurrence", "context"}, f"case {case['id']} candidate")
    require(candidate["fact"] == PROPERTY, f"case {case['id']} property differs")
    require(candidate["source_rule"] == "C735", f"case {case['id']} source rule differs")
    require(candidate["attr_occurrence"] in OCCURRENCES, f"case {case['id']} occurrence invalid")
    ...
    return {"id": case["id"], "kind": case["kind"], "attr_occurrence": candidate["attr_occurrence"], "context": candidate["context"], "computed": computed, "expected": case["expected"]}
...
def validate_binding(doc: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c735-source-backed-v0", "fixture schema differs")
    require(doc["origin"] == "HUMAN" and doc["property"] == PROPERTY, "fixture identity differs")
    source = doc["source"]
    require(source["document"] == "J3-24-007" and source["clause"] == "7" and source["rule"] == "C735" and source["printed_page"] == 88, "source identity differs")
    ...
    parts = lines[CANONICAL_LINE["line"] - 1].split(maxsplit=1)
    require(len(parts) == 2 and parts[0] == "22" and parts[1] == CANONICAL_LINE["text"], "canonical line differs")
    require(source["source_span"] == SOURCE_SPAN and canonical.read_bytes()[229534:229635] == SOURCE_BYTES, "source span differs")
    ...
    require("page 88 start 228013 length 2274" in page_index.read_text(encoding="utf-8"), "page index span differs")
    ...
    require(item["document"] == "J3-24-007" and item["clause"] == "7" and item["rule"] == "C735" and item["page"] == 88 and item["source_hash"] == SOURCE_SHA256, "semantic source identity differs")
    ...
    for fragment in ("(id S-C735)", "(subject " + PROPERTY + ")", "(origin human)", "(resolution disputed)"):
        require(fragment in text, f"semantic item lacks {fragment}")
    schema = (root / doc["contract"]["schema"]).read_text(encoding="utf-8")
    for fragment in ("(enum c735-attr-occurrence none distinct duplicate unknown)", "(enum c735-context-state derived-type-stmt other unknown)", "(record type-attr-uniqueness-use"):
        require(fragment in schema, f"contract schema lacks {fragment}")
    witness = (root / doc["contract"]["fixture"]).read_text(encoding="utf-8")
    for fragment in ("(contract m3-c735-derived-type-attr-uniqueness-oracle)", "(property " + PROPERTY + ")", "(rule C735)", "(page 88)", "(byte-start 229534)", "(byte-length 101)", "(source-hash " + SOURCE_SHA256 + ")"):
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, pdf: Path, canonical: Path, page_index: Path, standardir: Path, semantic: Path, semantic_output: Path, golden: Path, output: Path) -> dict[str, Any]:
    ...
    require({(case["attr_occurrence"], case["context"]) for case in cases} == {(occurrence, context) for occurrence in OCCURRENCES for context in CONTEXTS}, "12-state table incomplete")
    ...
    result = {
        ...
        "source": {"document": "J3-24-007", "clause": "7", "rule": "C735", "printed_page": 88, "pdf_sha256": digest(pdf), "canonical_text_sha256": digest(canonical), "page_index_sha256": digest(page_index), "standardir_sha256": digest(standardir), "canonical_lines": [3620], "source_span": SOURCE_SPAN, "page_index": {"path": ".cache/runs/E0001/R000003/j3-24-007.pages.index", "sha256": PAGE_INDEX_SHA256, "pages": [PAGE]}, "standardir_rules": ["R727", "R728"]},
        "semantic_items": {"input": "tests/fixtures/m3-c735-semantic-items.sx", "input_sha256": digest(semantic), "canonical_output_sha256": digest(semantic_output), "canonical_output": "PASS"},
        ...
        "model_calls": 0, "semantic_promotions": 0, "origin": "MECHANICAL", "oracle": "tests/e2e/validate_m3_c735.py",
    }
...
def self_test() -> None:
    for occurrence in OCCURRENCES:
        for context in CONTEXTS:
            expected = "ACCEPTED" if occurrence in {"none", "distinct"} and context == "derived-type-stmt" else "REJECTED" if occurrence == "duplicate" and context == "derived-type-stmt" else "UNRESOLVED"
            require(oracle({"attr_occurrence": occurrence, "context": context}) == expected, f"self-test failed for {occurrence}/{context}")


def main() -> int:
    if len(sys.argv) == 2 and sys.argv[1] == "--self-test":
        self_test()
        print("C735 oracle self-test PASS")
        return 0
    if len(sys.argv) != 9:
        raise SystemExit("usage: validate_m3_c735.py fixture semantic-output standardir canonical page-index pdf golden result")
    fixture_path, semantic_output, standardir, canonical, page_index, pdf, golden, output = map(Path, sys.argv[1:])
    root = Path(__file__).resolve().parents[2]
    try:
        fixture = json.loads(fixture_path.read_text(encoding="utf-8"))
        result = build_result(fixture_path, fixture, root, pdf, canonical, page_index, standardir, root / "tests/fixtures/m3-c735-semantic-items.sx", semantic_output, golden, output)
    except (ContractError, OSError, KeyError, TypeError, ValueError) as error:
        print(f"C735 oracle FAIL: {error}", file=sys.stderr)
        return 1
