...
oracle="$ROOT/tests/e2e/oracle_generated_print_expression_power_variable.py"
run_dir="$(mktemp -d)"

bash "$ROOT/scripts/check-contracts.sh" >/dev/null
bash "$ROOT/scripts/check_pins.sh" >/dev/null
bash "$ROOT/tests/e2e/check-generated-print-expression-power-literal.sh" >/dev/null
...
run_positive() {
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'power-variable ELF mutation was accepted: %s\n' "$source" >&2; exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'power-variable negative accepted: %s\n' "$source" >&2; exit 1
    fi
    [ ! -e "$ast" ] || { printf 'power-variable negative wrote AST: %s\n' "$source" >&2; exit 1; }
}

run_positive "$ROOT/tests/fixtures/l3-print-expression-power-variable-v0.f90" expression-power-variable-first
run_positive "$ROOT/tests/fixtures/l3-print-expression-power-variable-wide-v0.f90" expression-power-variable-second
...
printf '%s\n' 'generic power-variable-expression PASS'
