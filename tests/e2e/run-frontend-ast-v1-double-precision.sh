#!/usr/bin/env bash
# Replay the bounded source-derived DOUBLE PRECISION type-spec AST v1 contract.
...
manifest="${AST_MANIFEST:-$ROOT/tests/fixtures/frontend-ast-v1-double-precision-replay.toml}"
contract_validator="$ROOT/tests/e2e/validate_frontend_ast_v1_double_precision_contract.py"
validator="$ROOT/tests/e2e/validate_frontend_ast_v1_double_precision.py"
trace_writer="$ROOT/tests/e2e/write_frontend_ast_v1_double_precision_trace.py"
frontend="$(resolve_repo fortfront-new)"
run_root="${AST_RUN_ROOT:-$ROOT/.cache/runs/E0243}"
run_number=1
...
if manifest["validator"] != "tests/e2e/validate_frontend_ast_v1_double_precision.py":
    raise SystemExit("validator path is not the frozen validator")
if manifest["trace_writer"] != "tests/e2e/write_frontend_ast_v1_double_precision_trace.py":
...
python3 "$contract_validator" tests/fixtures/frontend-ast-v1-double-precision-contract.toml >"$run_dir/contract-oracle.log"
...
if (cd "$frontend" && fo exec fortfront-source-ast-v1 "$negative" "$run_dir/negative.ast.sx") >"$run_dir/negative.log" 2>&1; then
    printf '%s\n' 'malformed DOUBLE PRECISION declaration was accepted' >&2
    exit 1
fi
...
if [ -f "$committed_trace" ]; then
    ...
    [ "${AST_BOOTSTRAP_TRACE:-}" = 1 ] || { printf '%s\n' 'missing committed DOUBLE PRECISION AST trace; use AST_BOOTSTRAP_TRACE=1 once' >&2; exit 1; }
    cp "$run_dir/trace.json" "$committed_trace"
fi
printf 'frontend AST v1 DOUBLE PRECISION PASS: %s\n' "$run_dir"
