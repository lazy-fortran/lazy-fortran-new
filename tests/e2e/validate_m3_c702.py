#!/usr/bin/env python3
"""Independent validator for the bounded M3 C702 semantic slice."""
...
CONTEXTS = {"pointer", "allocatable", "neither", "unknown"}
...
PROPERTY = "type-param-colon-attribute-legality"
...
def c702_oracle(candidate: dict[str, Any]) -> str:
    """The deliberately small decision procedure for C702."""

    context = candidate["attribute_context"]
    if context == "unknown":
        return "UNRESOLVED"
    if context in {"pointer", "allocatable"}:
        return "ACCEPTED"
    return "REJECTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"type", "source_rule", "attribute_context"}, f"case {case['id']} candidate")
    require(candidate["type"] == "type-param-colon-use", f"case {case['id']} candidate type differs")
    require(candidate["source_rule"] == "C702", f"case {case['id']} source rule differs")
    require(candidate["attribute_context"] in CONTEXTS, f"case {case['id']} attribute context is invalid")
    computed = c702_oracle(candidate)
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(enum attribute-context pointer allocatable neither unknown)" in schema, "contract schema lacks attribute contexts")
    require("(record type-param-colon-use" in schema, "contract schema lacks typed colon fact")
    require("(attribute-context attribute-context)" in schema, "contract schema lacks typed context field")


def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    text = (root / doc["contract"]["fixture"]).read_text(encoding="utf-8")
    for fragment in (
        "(contract m3-c702-semantic-oracle)",
        "(property type-param-colon-attribute-legality)",
        "(document J3-24-007)",
        "(clause 7)",
        "(rule C702)",
...
def validate_source_binding(
    ...
    require(doc["schema_version"] == "m3-c702-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C702", "source rule differs")
    require(source["printed_page"] == 62, "source printed page differs")
...
def build_result(
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c702.py",
    }


def self_test() -> None:
    cases = [
        ({"type": "type-param-colon-use", "source_rule": "C702", "attribute_context": "pointer"}, "ACCEPTED"),
        ({"type": "type-param-colon-use", "source_rule": "C702", "attribute_context": "allocatable"}, "ACCEPTED"),
        ({"type": "type-param-colon-use", "source_rule": "C702", "attribute_context": "neither"}, "REJECTED"),
        ({"type": "type-param-colon-use", "source_rule": "C702", "attribute_context": "unknown"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c702_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C702 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 8:
        print(
            "usage: validate_m3_c702.py FIXTURE SEMANTIC_OUTPUT STANDARDIR "
    ...
    try:
        ...
        print(f"M3 C702 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C702 validator: PASS")
    return 0
