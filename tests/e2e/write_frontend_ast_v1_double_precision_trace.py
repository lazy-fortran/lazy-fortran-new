#!/usr/bin/env python3
"""Write the canonical trace for the bounded DOUBLE PRECISION replay."""
...
def main() -> int:
    if len(sys.argv) != 4:
        raise SystemExit("usage: write_frontend_ast_v1_double_precision_trace.py replay-manifest run-dir frontend")
    ...
    for case in contract["case"]:
        ...
        cases.append({
            ...
            "expected_variable_type": case["expected_variable_type"],
            "ast_sha256": digest(run_dir / f"case-{case['id']}.ast.sx", source),
        })
    rejection_marker = "typed frontend rejected source:"
    if rejection_marker not in (run_dir / "negative.log").read_text(encoding="utf-8"):
    ...
    trace = {
        ...
        "cases": cases,
        "negative": {"id": "missing-entity", "expected_outcome": "REJECTED", "rejection_marker": rejection_marker},
