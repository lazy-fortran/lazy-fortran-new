...
oracle="$ROOT/tests/e2e/oracle_generated_print_expression.py"
run_dir="$(mktemp -d)"
...
run_positive() {
    ...
    if python3 "$oracle" "$ast" "$mir" "$mutated_elf" "$source" >/dev/null 2>&1; then
        printf 'expression ELF mutation was accepted: %s\n' "$source" >&2
        exit 1
    fi
}
...
run_negative() {
    ...
    if (cd "$frontend" && fo exec fortfront-program-unit-v2 "$source" "$ast") >/dev/null 2>&1; then
        printf 'expression negative accepted: %s\n' "$source" >&2
        exit 1
    fi
    [ ! -e "$ast" ] || {
        printf 'expression negative wrote AST: %s\n' "$source" >&2
        exit 1
    }
}

run_positive "$ROOT/tests/fixtures/l3-print-expression-v0.f90" expression-first
run_positive "$ROOT/tests/fixtures/l3-print-expression-wide-v0.f90" expression-second
run_negative "$ROOT/tests/negative/l3-print-expression-v0-missing-operand.f90" missing-operand
run_negative "$ROOT/tests/negative/l3-print-expression-v0-wrong-operator.f90" wrong-operator
run_negative "$ROOT/tests/negative/l3-print-expression-v0-write.f90" write
run_negative "$ROOT/tests/negative/l3-print-expression-v0-wrong-name.f90" wrong-name

printf '%s\n' 'generic print-expression PASS'
