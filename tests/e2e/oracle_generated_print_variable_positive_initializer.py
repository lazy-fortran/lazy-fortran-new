#!/usr/bin/env python3
"""Independent oracle for bounded positive variable initialization and PRINT."""
...
def source_value(path: pathlib.Path) -> str:
    ...
    match = re.search(r"^  x = ([0-9]+)$", text, flags=re.MULTILINE)
    require(match is not None, "source initializer differs")
    value = match.group(1)
    require(0 <= int(value) <= 2047, "source initializer is out of bounded range")
...
def check_mir(path: pathlib.Path, value: str) -> None:
    ...
    require("(storage-key x)" in instructions[1], "MIR store key differs")
    require("(storage-key x)" in instructions[2], "MIR load key differs")
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_positive_initializer.py AST MIR ELF SOURCE")
    ...
    print("positive variable initializer oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"positive variable initializer oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
