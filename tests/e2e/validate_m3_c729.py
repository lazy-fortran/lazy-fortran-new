#!/usr/bin/env python3
"""Independent validator for the bounded M3 C729 semantic slice."""
...
COMMA_STATES = {"absent", "present", "unknown"}
CONTEXT_STATES = {"allowed", "other", "unknown"}
...
PROPERTY = "length-selector-optional-comma-legality"
...
def c729_oracle(candidate: dict[str, Any]) -> str:
    """Apply only C729's optional-comma context implication."""

    comma = candidate["comma_state"]
    context = candidate["context_state"]
    if comma == "unknown" or context == "unknown":
        return "UNRESOLVED"
    if comma == "absent":
        return "ACCEPTED"
    return "ACCEPTED" if context == "allowed" else "REJECTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"type", "source_rule", "comma_state", "context_state"}, f"case {case['id']} candidate")
    require(candidate["type"] == "optional-comma-use", f"case {case['id']} candidate type differs")
    require(candidate["source_rule"] == "C729", f"case {case['id']} source rule differs")
    require(candidate["comma_state"] in COMMA_STATES, f"case {case['id']} comma state is invalid")
    require(candidate["context_state"] in CONTEXT_STATES, f"case {case['id']} context state is invalid")
    computed = c729_oracle(candidate)
    require(computed == case["expected"], f"case {case['id']} expected label disagrees with oracle")
    return {"id": case["id"], "kind": case["kind"], "computed": computed, "expected": case["expected"]}
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(enum comma-state absent present unknown)" in schema, "contract schema lacks comma states")
    require("(enum context-state allowed other unknown)" in schema, "contract schema lacks context states")
    require("(record optional-comma-use" in schema, "contract schema lacks typed comma fact")
    require("(comma-state comma)" in schema and "(context-state context)" in schema, "contract schema lacks typed fields")


def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    text = (root / doc["contract"]["fixture"]).read_text(encoding="utf-8")
    for fragment in (
        "(contract m3-c729-optional-comma-oracle)",
        "(property length-selector-optional-comma-legality)",
        "(document J3-24-007)",
        "(clause 7)",
        "(rule C729)",
...
def validate_source_binding(doc: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, standardir_path: Path, semantic_input: Path) -> None:
    ...
    require(doc["schema_version"] == "m3-c729-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C729", "source rule differs")
...
def build_result(fixture_path: Path, fixture: dict[str, Any], root: Path, source_pdf: Path, canonical_path: Path, standardir_path: Path, semantic_input: Path, semantic_output: Path, golden: Path) -> dict[str, Any]:
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c729.py",
    }


def self_test() -> None:
    cases = [
        ({"type": "optional-comma-use", "source_rule": "C729", "comma_state": "present", "context_state": "allowed"}, "ACCEPTED"),
        ({"type": "optional-comma-use", "source_rule": "C729", "comma_state": "absent", "context_state": "other"}, "ACCEPTED"),
        ({"type": "optional-comma-use", "source_rule": "C729", "comma_state": "present", "context_state": "other"}, "REJECTED"),
        ({"type": "optional-comma-use", "source_rule": "C729", "comma_state": "present", "context_state": "unknown"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c729_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C729 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    if argv[1:] == ["--self-test"]:
        ...
        return 0
    if len(argv) != 8:
        print("usage: validate_m3_c729.py FIXTURE SEMANTIC_OUTPUT STANDARDIR CANONICAL_TEXT PDF GOLDEN RESULT_JSON", file=sys.stderr)
        return 2
    ...
    try:
        ...
        print(f"M3 C729 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C729 validator: PASS")
    return 0
