#!/usr/bin/env python3
"""Independent validator for the bounded M3 C725 semantic slice."""
...
PROPERTY = "int-literal-kind-legality"
EXPECTED_CANONICAL_LINES = [
    {"line": 3452, "text": "C725 (R723) The int-literal-constant shall not include a kind-param."}
]
EXPECTED_STANDARDIR_ROWS = [
    {"rule": "R723", "lhs": "char-length", "page": 84, "byte_start": 217536, "byte_length": 70, "occurrence": 73},
    {"rule": "R708", "lhs": "int-literal-constant", "page": 80, "byte_start": 207585, "byte_length": 61, "occurrence": 58},
]
...
def c725_oracle(candidate: dict[str, Any]) -> str:
    """Apply C725 to the typed R723/R708 kind-parameter state."""

    kind_param = candidate["kind_param"]
    if kind_param not in {"present", "absent"}:
        return "UNRESOLVED"
    return "REJECTED" if kind_param == "present" else "ACCEPTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"type", "source_rule", "kind_param"}, f"case {case['id']} candidate")
    require(candidate["type"] == "int-literal-kind-use", f"case {case['id']} candidate type differs")
    require(candidate["source_rule"] == "C725", f"case {case['id']} source rule differs")
    require_string(candidate["kind_param"], f"case {case['id']} kind-param state")
    computed = c725_oracle(candidate)
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(record int-literal-kind-use" in schema, "contract schema lacks typed int-literal fact")
...
def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in (
        "(contract m3-c725-int-literal-kind-oracle)",
        "(property int-literal-kind-legality)",
        "(document J3-24-007)",
        "(clause 7)",
        "(rule C725)",
...
def validate_source_binding(
    ...
    require(doc["schema_version"] == "m3-c725-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C725", "source rule differs")
    require(source["printed_page"] == 83, "source printed page differs")
...
def build_result(
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c725.py",
    }


def self_test() -> None:
    cases = [
        ({"type": "int-literal-kind-use", "source_rule": "C725", "kind_param": "absent"}, "ACCEPTED"),
        ({"type": "int-literal-kind-use", "source_rule": "C725", "kind_param": "present"}, "REJECTED"),
        ({"type": "int-literal-kind-use", "source_rule": "C725", "kind_param": "unknown"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c725_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C725 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 8:
        print(
            "usage: validate_m3_c725.py FIXTURE SEMANTIC_OUTPUT STANDARDIR "
    ...
    try:
        ...
        print(f"M3 C725 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C725 validator: PASS")
    return 0
