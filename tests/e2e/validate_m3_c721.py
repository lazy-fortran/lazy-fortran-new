#!/usr/bin/env python3
"""Independent validator for the bounded M3 C721 semantic slice."""
...
PROPERTY = "exponent-letter-legality"
EXPECTED_CANONICAL_LINES = [
    {"line": 3355, "text": "C721 (R714) If both kind-param and exponent-letter appear, exponent-letter shall be E."}
]
EXPECTED_STANDARDIR_ROWS = [
    {"rule": "R714", "lhs": "real-literal-constant", "page": 82, "byte_start": 211120, "byte_length": 151, "occurrence": 64},
    {"rule": "R716", "lhs": "exponent-letter", "page": 82, "byte_start": 211348, "byte_length": 36, "occurrence": 66},
]
...
def c721_oracle(candidate: dict[str, Any]) -> str:
    """Apply the C721 implication to typed R714/R716 relation fields."""

    kind_param = candidate["kind_param"]
    exponent_letter = candidate["exponent_letter"]
    if kind_param not in {"present", "absent"}:
        return "UNRESOLVED"
    if exponent_letter not in {"none", "E", "D"}:
        return "UNRESOLVED"
    return "REJECTED" if kind_param == "present" and exponent_letter == "D" else "ACCEPTED"
...
def validate_candidate(case: dict[str, Any]) -> dict[str, Any]:
    ...
    exact_keys(candidate, {"type", "source_rule", "kind_param", "exponent_letter"}, f"case {case['id']} candidate")
    require(candidate["type"] == "exponent-letter-use", f"case {case['id']} candidate type differs")
    require(candidate["source_rule"] == "C721", f"case {case['id']} source rule differs")
    require_string(candidate["kind_param"], f"case {case['id']} kind-param state")
    require_string(candidate["exponent_letter"], f"case {case['id']} exponent-letter state")
    computed = c721_oracle(candidate)
...
def validate_contract_schema(doc: dict[str, Any], root: Path) -> None:
    ...
    require("(record exponent-letter-use" in schema, "contract schema lacks typed exponent-letter fact")
    require("(kind-param kind-param-state)" in schema, "contract schema lacks kind-param state")
    require("(exponent-letter exponent-letter-state)" in schema, "contract schema lacks exponent-letter state")
...
def validate_contract_fixture(doc: dict[str, Any], root: Path) -> None:
    ...
    for fragment in (
        "(contract m3-c721-exponent-letter-oracle)",
        "(property exponent-letter-legality)",
        "(document J3-24-007)",
        "(clause 7)",
        "(rule C721)",
...
def validate_source_binding(
    ...
    require(doc["schema_version"] == "m3-c721-source-backed-v0", "fixture schema version differs")
    ...
    require(source["rule"] == "C721", "source rule differs")
    require(source["printed_page"] == 81, "source printed page differs")
...
def build_result(
    ...
    return {
        ...
        "origin": "MECHANICAL",
        "oracle": "tests/e2e/validate_m3_c721.py",
    }


def self_test() -> None:
    cases = [
        ({"type": "exponent-letter-use", "source_rule": "C721", "kind_param": "present", "exponent_letter": "E"}, "ACCEPTED"),
        ({"type": "exponent-letter-use", "source_rule": "C721", "kind_param": "absent", "exponent_letter": "D"}, "ACCEPTED"),
        ({"type": "exponent-letter-use", "source_rule": "C721", "kind_param": "present", "exponent_letter": "D"}, "REJECTED"),
        ({"type": "exponent-letter-use", "source_rule": "C721", "kind_param": "present", "exponent_letter": "unknown"}, "UNRESOLVED"),
    ]
    for candidate, expected in cases:
        require(c721_oracle(candidate) == expected, f"self-test outcome differs: {expected}")
    print("M3 C721 oracle self-test: PASS")
...
def main(argv: list[str]) -> int:
    ...
    if len(argv) != 8:
        print(
            "usage: validate_m3_c721.py FIXTURE SEMANTIC_OUTPUT STANDARDIR "
    ...
    try:
        ...
        print(f"M3 C721 validator: FAIL: {error}", file=sys.stderr)
        return 1
    print("M3 C721 validator: PASS")
    return 0
