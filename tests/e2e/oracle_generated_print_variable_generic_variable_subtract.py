...
def check_mir(path: pathlib.Path) -> None:
    ...
    expected = ["const", "store", "load", "load", "sub", "store", "load", "output", "return"]
...
def main() -> int:
    if len(sys.argv) != 5:
        raise OracleFailure("usage: oracle_generated_print_variable_generic_variable_subtract.py AST MIR ELF SOURCE")
    ...
    print("generic initialized variable subtract oracle PASS")
    return 0
...
if __name__ == "__main__":
    try:
        ...
        print(f"generic initialized variable subtract oracle failure: {error}", file=sys.stderr)
        raise SystemExit(1)
