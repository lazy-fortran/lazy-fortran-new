...
| 2026-08-16 | `lfortran/lfortran` | BSD-3-Clause | Pinned `src/lfortran/parser/parser.yy` at commit `caf87b660f803148f000046392a5da803f9fc630`, especially `statement`, `sep`, `sep_one`, and the `TK_NEWLINE`/`TK_COMMENT`/`TK_EOLCOMMENT` terminals; `src/lfortran/parser/tokenizer.re` and `src/lfortran/parser/fixedform_tokenizer.cpp` for trivia and statement-boundary tokenization | E0171 statement-boundary design review: separators are lexer tokens consumed by a generic statement boundary, not a suffix appended to every `-stmt`; no productions or actions imported |
