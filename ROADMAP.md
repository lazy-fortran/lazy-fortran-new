...
**Current goal-mode handoff.** E0147 is now terminal as `R000267` at
`standard-new` commit `17525dbef6ec7a44ee701bc400841e8a907708df`. The
source-backed validity pipeline reconstructs page-crossing productions,
preserves occurrence and source-line identity, lexes attached punctuation and
terminal/reference transitions, runs independent positive and negative
controls, and regenerates EBNF, ANTLR4, Bison and tree-sitter only from the
validated SX. Its terminal report is regenerated by:
...
The immediate source-validity gate is closed. Reference closure and semantic
promotion are not. Do not run E0142 or any older model matrix. The next lane
is deterministic classification of the remaining reference classes and
explicit compact errata, followed only by a bounded Qwen 3.8 27B residual lane
with source-span and replay witnesses. After any source-valid generated
artifact, D0085 requires one independent native GPT-5.6 Luna review with the
...
1. Keep the E0147 source-valid syntax baseline immutable and retain its
   terminal report and generated exports.
2. Mechanically classify every remaining reference as production, lexical fact,
   assumed syntax, fixed erratum, semantic-only or unresolved; require a
   source witness for each classification.
3. Add only compact, source-backed errata records for documented
   inconsistencies, keeping them separate from the normative baseline.
4. Run the Luna review and use its findings only to improve context,
   controls, and harness protocol.
5. Give Qwen 3.8 27B only the bounded residue that survives those gates;
   accept a proposal only after deterministic replay and an independent
   source-span witness.
6. Promote the closed syntax/reference layer into semantic extraction, then
