...
conflicts; regenerate them with the same replay or
`validate-grammar-exports.sh <run-directory>`. They are retained evidence of
the target parser's ambiguity, not source-structure loss. EBNF remains a
projection-only format without an independent parser validator. The grammar
source/provenance subgate therefore passes, but the selected grammar
milestone is not called unqualified or conflict-free.
