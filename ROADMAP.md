...
Luna has confirmed the bounded claim. No model run,
...
(D0115); the next gate is mapping those 95 source sites through target
...
1. Map the R000429/R000430 source results through each target expression tree
   without changing the raw-source identity. Preserve explicit `mapped`,
