...
The maintenance wave `T-WAVE-C-component-suite-repair` is complete. Its four
disjoint test-only scopes reconcile retained full-suite failures against the
current generated policies and bounded route oracles. The controller verifier
`scripts/check_pins.sh && scripts/check-contracts.sh && bash
tests/e2e/check-generated-chain.sh` passes. StandardIR retains six unrelated
...
Wave D, `T-WAVE-D-ffc-suite-repair`, is complete: two disjoint test-only
scopes reconcile the remaining FFC full-suite negative controls at
`8bfa92f205a42f474653aaa4350ac03d685f19c7`. `fo clean && fo`, `fo test --all`
and the controller verifier
`scripts/check_pins.sh && scripts/check-contracts.sh && bash
...
and ten-item literal lists through AST-v2, typed MIR, RISC-V and qemu, rejects
...
remain open.
The initialized-power predecessor remains integrated. Its central command
`bash tests/e2e/check-generated-print-variable-generic-power.sh` checks
`x = 3; x = x ** 2` and `x = -3; x = x ** 3` through AST-v2, exact
nine-instruction MIR, generated RISC-V and qemu, while exponents outside the
generated 2..10 range, real and malformed source neighbours, and AST/MIR/ELF
mutations reject. The range successor extends this predecessor; both remain
source-driven bounded power transport. General expression parsing, full power
semantics and semantic promotion remain open.
The initialized-power range successor is now integrated. Its central command
`bash tests/e2e/check-generated-print-variable-generic-power-range.sh` checks
`x = 3; x = x ** 5`, `x = -3; x = x ** 5` and `x = 3; x = x ** 10` through
AST-v2, exact nine-instruction MIR, generated RISC-V and qemu, while exponents
0, 1 and 11, real and malformed neighbours, and AST/MIR/ELF mutations reject.
...
The initialized variable-multiply successor is now integrated. Its central
command `bash tests/e2e/check-generated-print-variable-generic-variable-multiply.sh`
checks `x = 3; x = x * x` and `x = 4; x = x * x` through the generated
frontend, typed MIR, RISC-V and qemu, with exact outputs `9` and `16`; a signed
out-of-range initializer, real initializer, wrong-name/operator/RHS neighbours,
and AST/MIR/ELF mutations reject. This remains a bounded initialized
variable-multiply witness, not general expression parsing or semantic
promotion.
The initialized variable-divide successor is now integrated. Its central
command `bash tests/e2e/check-generated-print-variable-generic-variable-divide.sh`
checks `x = 3; x = x / x` and `x = -4; x = x / x` through the generated
frontend, typed MIR, RISC-V and qemu, with exact outputs `1` and `1`; a signed
out-of-range initializer, real initializer, wrong-name/operator/RHS neighbours,
and AST/MIR/ELF mutations reject. This remains a bounded initialized
