...
E0147's deterministic closure/export implementation is now complete in
`standard-new` commit `65b38f4`. The replay emits 1,222 closed grammar rules
from 522 source syntax records, and ANTLR4, Bison and tree-sitter all accept
the generated outputs. The Bison report retains its 127 shift/reduce and 844
...
negative controls. The generated ANTLR4, Bison and tree-sitter files are
usable parser-generator inputs. Bison's intrinsic ambiguity and unreachable
record diagnostics remain recorded for later grammar-quality analysis; they
are not unresolved-name or fatal-generation failures.
...
The source-validity, closure and fatal-validator gates are now closed for this
selected profile. Do not resume E0142 or any older model matrix. The next
required step is D0085's independent native GPT-5.6 Luna review with the
minimal prompt “How well does this represent the Fortran 2023 standard?”.
That review may identify grammar-quality refinements, but it cannot rewrite,
validate, or promote StandardIR. Semantic and model work remains stopped until
that review and any resulting deterministic fixes are recorded.
...
4. Run ANTLR4, Bison and tree-sitter in the same command, retain complete logs,
   and require zero unresolved-symbol or fatal-generation outcomes. Preserve
   Bison conflict and reachability warnings as named metrics; do not hide them
   by pruning rules or suppressing diagnostics.
