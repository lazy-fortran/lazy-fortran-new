...
accepted as `R000267`; its first closure/export acceptance is superseded by
`R000270`. The corrected generator is pushed as `standard-new` `e6a03b6`, and
the exact committed replay is `R000009`. Its ANTLR4, Bison and tree-sitter
validators pass and its negative mutation fails, but those checks do not prove
source fidelity. The next required work is a generic source-to-projection
equivalence witness, followed by Luna's independent read-only review of the
SX input and EBNF, ANTLR4, Bison and tree-sitter outputs. No semantic or model
work resumes until both are recorded as passing.
...
- [ ] E0147: close the source-backed StandardIR validity audit, reference
      ...
      source-validity subgate; `R000269` is superseded by `R000270` after Luna
      found collapsed alternatives. `standard-new` `e6a03b6` and replay
      `R000009` are the current correction; the equivalence witness and fresh
      Luna review remain pending.
