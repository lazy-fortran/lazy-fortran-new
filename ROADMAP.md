...
**Current goal-mode handoff.** E0147's source-validity subgate is accepted as
`R000267` at `standard-new` commit
`17525dbef6ec7a44ee701bc400841e8a907708df`; E0147 itself remains open under
D0086. The source-backed pipeline reconstructs page-crossing productions,
preserves occurrence and source-line identity, lexes attached punctuation and
terminal/reference transitions, and runs independent positive and negative
controls. It has not yet produced a usable parser grammar: the first ANTLR4,
Bison and tree-sitter oracle run rejected the exports on unresolved references.
...
The grammar-oracle procedure is:

    research/experiments/E0147-can-source-backed-standardir-validity-close/validate-grammar-exports.sh <run-directory>

The immediate source-validity gate is closed, but reference closure and all
grammar-validator gates are blockers. Do not run E0142 or any older model
matrix. The next lane is one fully automatic closure and target-normalization
pipeline using only source-backed R401/R402/R403 facts and generic grammar
algorithms. It must make ANTLR4, Bison and tree-sitter accept the same output;
manual target edits and R-number exceptions are invalid. Semantic and model
work remains stopped. After the complete source-valid generated artifact is
accepted, D0085 requires one independent native GPT-5.6 Luna review with the
minimal prompt “How well does this represent the Fortran 2023 standard?”.
That review may refine the harness and add deterministic controls, but never
rewrites, validates, or promotes StandardIR.

**Current success path.**

1. Keep the E0147 source-valid syntax baseline immutable and retain its
   accepted subgate report and failed validator evidence.
2. Mechanically classify every remaining reference as production, lexical fact,
   R401/R402/R403 assumed syntax, fixed erratum, semantic-only or unresolved;
   require a source witness and a deterministic closure fixed point.
3. Generate target grammars only from that closure. Apply only generic
   duplicate, nullable and left-recursion transformations, with provenance
   mappings; never hand-edit a generated grammar.
4. Run ANTLR4, Bison and tree-sitter in the same command, retain complete logs,
   and require zero unresolved-symbol or fatal-generation outcomes.
5. Add only compact, source-backed errata records for documented
   inconsistencies, keeping them separate from the normative baseline.
6. Run the Luna review only after the complete deterministic gate and use its
   findings only to improve context, controls, and harness protocol.
7. Resume Qwen 3.8 27B only on a bounded residue that survives all gates;
   accept a proposal only after deterministic replay and an independent
   source-span witness.
8. Promote the closed syntax/reference layer into semantic extraction, then
...
- [ ] E0147: close the source-backed StandardIR validity audit, reference
      closure, and ANTLR4/Bison/tree-sitter validator gates. `R000267` is only
      its accepted source-validity subgate; no model calls are allowed here.
