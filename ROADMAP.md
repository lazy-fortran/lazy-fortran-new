...
`bash tests/e2e/check-generated-chain.sh` passes 151 routes. General
...
remain open.
The same raw route now carries `counter_2 = 42` followed by
...
The same path now accepts `counter_2 = counter_2 / 2` with exact `21` output.
The central oracle checks the division opcode and divisor mutation. Broader
expression coverage remains open.
