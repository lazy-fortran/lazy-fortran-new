#!/usr/bin/env bash
# Verify generated schema validation and equality against fixed semantic cases.

set -euo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
standard="${STANDARD_NEW:-$root/../standard-new}"
expected_commit="531e590"
outdir="${1:-$root/.cache/runs/E0040/R000001}"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
run_check driver fo exec sxschema specs/schema-v0.sxs \
    build/E0040-regenerated.f90 schema_v0_generated
run_check generated_byte_identity cmp generated/schema_v0_generated.f90 \
    build/E0040-regenerated.f90
...
printf 'valid_values_checked\t7\n' >>"$outdir/summary.tsv"
printf 'invalid_values_checked\t5\n' >>"$outdir/summary.tsv"
printf 'equality_cases_checked\t8\n' >>"$outdir/summary.tsv"
...
if awk -F '\t' '$2 != 0 {failed=1} END {exit failed}' "$tmp/status.tsv"; then
    printf 'E0040 oracle: generated validators and equality agreed with fixed semantic cases\n'
    cat "$outdir/summary.tsv"
else
    printf 'E0040 failed: one or more checks did not pass\n' >&2
    exit 1
fi
