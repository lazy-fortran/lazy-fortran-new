#!/usr/bin/env bash
# Generate and execute precedence-shaped expression trees.

set -euo pipefail
export LC_ALL=C

root="$(cd "$(dirname "$0")/../../.." && pwd)"
corpus="$root/research/corpora/phase1-modern-fortran-precedence-trees-v0.json"
...
outdir="${1:-$root/.cache/runs/E0066/R000001}"
...
jq -e '.name == "phase1-modern-fortran-precedence-trees-v0" and (.witnesses | length) == 7 and ([.witnesses[].expected_internal_nodes] | add) == 10 and ([.witnesses[].expected_leaf_nodes] | add) == 17' "$corpus" >/dev/null || die 'precedence corpus manifest shape differs'
...
array_nodes="$(jq '[.witnesses[].kind_counts["array-constructor"]] | add' "$corpus")"
...
cat >"$outdir/generated_precedence_trees.f90" <<'EOF'
module generated_precedence_trees
    ...
    public :: build_precedence_tree, find_precedence_root

contains

    subroutine build_precedence_tree(path, role, first_line, last_line, nodes, base_count, &
        ...
        token_position = 1
        call parse_relational(role_id, first_line, last_line, nodes, node_count, internal_count, &
    ...
    end subroutine build_precedence_tree

    subroutine find_precedence_root(nodes, node_count, role, first_line, last_line, node_id, found)
        ...
        do i = 1, node_count
            ...
            do j = 1, node_count
                if (nodes(j)%parent == i .and. (trim(nodes(j)%kind) == "binary-expr" .or. &
                    trim(nodes(j)%kind) == "unary-expr" .or. trim(nodes(j)%kind) == "array-constructor")) then
                    node_id = j
                    found = .true.
                    return
                end if
    ...
    end subroutine find_precedence_root
            ...
            if (current == " " .or. current == char(9)) then
                ...
                do while (j <= length)
                    ...
                    if (.not. ((current >= "0" .and. current <= "9") .or. current == "." .or. &
                              current == "e" .or. current == "E")) exit
                    j = j + 1
                end do
                if (index(text(i:j - 1), ".") > 0) then
                ...
                if ((current == "<" .or. current == ">" .or. current == "=" .or. current == "/") .and. &
        ...
        if (token_is("name") .or. token_is("literal")) then
            if (token_is("name")) then
                call append_leaf("name", "name", "R603", first_line, last_line, nodes, node_count, &
                                 leaf_count, result_id, link_errors)
            ...
            do while (.not. token_is("rbracket"))
                call parse_relational(new_id, first_line, last_line, nodes, node_count, internal_count, &
                                      leaf_count, link_errors, child_id, ierr)
                if (ierr /= 0) return
                call attach_child(new_id, child_id, nodes, link_errors)
...
end module generated_precedence_trees
EOF
...
{
    printf '%s\n' 'program test_generated_precedence_trees'
    printf '%s\n' '    use generated_precedence_trees, only: build_precedence_tree, find_precedence_root'
    printf '%s\n' '    use generated_ast_records, only: ast_node_t'
    printf '%s\n' '    implicit none'
    printf '%s\n' '    integer :: precedence_query_hits, unknown_query_rejected'
    printf '%s\n' '    precedence_query_hits = 0; unknown_query_rejected = 0'
    while IFS=$'\t' read -r relative_path first_line last_line role expected_base expected_internal expected_leaf expected_new expected_depth root_rule expected_binary expected_unary expected_array expected_name expected_literal; do
        printf "    call check_witness('%s/%s', %s, %s, '%s', %s, %s, %s, %s, %s, '%s', %s, %s, %s, %s, %s)\n" "$fortfront_root" "$relative_path" "$first_line" "$last_line" "$role" "$expected_base" "$expected_internal" "$expected_leaf" "$expected_new" "$expected_depth" "$root_rule" "$expected_binary" "$expected_unary" "$expected_array" "$expected_name" "$expected_literal"
    done < <(jq -r '.witnesses[] | [.path, .start_line, .end_line, .role, .expected_base_nodes, .expected_internal_nodes, .expected_leaf_nodes, .expected_new_nodes, .expected_max_depth, .root_rule, .kind_counts["binary-expr"], .kind_counts["unary-expr"], .kind_counts["array-constructor"], .kind_counts.name, .kind_counts.literal] | @tsv' "$corpus")
    printf '%s\n' '    if (precedence_query_hits /= 7 .or. unknown_query_rejected /= 1) error stop "query summary mismatch"'
    printf '%s\n' '    call check_mutation()'
    printf '%s\n' '    print "(a,i0,1x,a,i0)", "precedence queries: ", precedence_query_hits, "unknown rejected: ", unknown_query_rejected'
    printf '%s\n' 'contains'
    printf '%s\n' '    subroutine check_witness(path, first_line, last_line, role, expected_base, expected_internal, expected_leaf, expected_new, expected_depth, root_rule, expected_binary, expected_unary, expected_array, expected_name, expected_literal)'
    ...
    printf '%s\n' '        integer, intent(in) :: expected_binary, expected_unary, expected_array, expected_name, expected_literal'
    ...
    printf '%s\n' '        logical :: found'
    printf '%s\n' '        call build_precedence_tree(path, role, first_line, last_line, parsed, base_count, actual_internal, actual_leaf, actual_total, root_id, errors, depth, status)'
    printf '%s\n' '        if (status /= 0 .or. base_count /= expected_base .or. actual_internal /= expected_internal .or. actual_leaf /= expected_leaf .or. actual_total /= expected_base + expected_new) error stop "precedence tree shape mismatch"'
    printf '%s\n' '        if (errors /= 0 .or. depth /= expected_depth .or. root_id <= expected_base) error stop "precedence tree links mismatch"'
    ...
    printf '%s\n' '        call find_precedence_root(parsed, actual_total, role, first_line, last_line, j, found)'
    printf '%s\n' '        if (.not. found .or. j /= root_id) error stop "precedence query mismatch"'
    printf '%s\n' '        precedence_query_hits = precedence_query_hits + 1'
    printf '%s\n' '        call find_precedence_root(parsed, actual_total, "unknown-role", first_line, last_line, j, found)'
    printf '%s\n' '        if (found) error stop "unknown precedence query accepted"'
    printf '%s\n' '        if (precedence_query_hits == 1) unknown_query_rejected = 1'
    ...
    printf "        call build_precedence_tree('%s', 'expr', 8, 8, mutated, base_count, internal, leaves, total, root_id, errors, depth, status)\n" "$mutation"
    printf '%s\n' '        if (status == 0) error stop "malformed nesting accepted"'
    printf '%s\n' '    end subroutine check_mutation'
    printf '%s\n' 'end program test_generated_precedence_trees'
} >"$outdir/test_generated_precedence_trees.f90"

set +e
gfortran -ffree-line-length-none -Wall -Wextra -Werror "$diagnostic_module" "$logical_module" "$ast_module" "$expression_module" "$outdir/generated_precedence_trees.f90" "$outdir/test_generated_precedence_trees.f90" -o "$outdir/test_generated_precedence_trees" >"$outdir/fortran.log" 2>&1
fortran_compile_status=$?
if test "$fortran_compile_status" -eq 0; then
    "$outdir/test_generated_precedence_trees" >"$outdir/runtime.log" 2>&1
    runtime_test_status=$?
else
...
if test "$fortran_compile_status" -eq 0 && test "$runtime_test_status" -eq 0; then
    ...
    precedence_query_hits="$witness_count"
...
if test "$source_linked_nodes" -eq $((internal_nodes + leaf_nodes)) && \
   ...
   test "$precedence_query_hits" -eq "$witness_count" && test "$unknown_query_rejected" -eq 1; then
    target_boundary="source_linked_precedence_trees_validated"
    tree_mismatches=0
else
    target_boundary="verification_failure_source_linked_precedence_trees"
    tree_mismatches=1
fi
...
printf 'E0066 oracle: generated precedence tree operation completed\n'
