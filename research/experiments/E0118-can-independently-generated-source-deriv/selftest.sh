...
test "$(jq -r '.source_case_oracle_mismatches' "$outdir/summary.json")" = 0
...
test "$(jq -s -r '[.[] | select(.row_key == "F001@1" and .source_expected == true and .facts.x == 1)] | length' "$outdir/cases.jsonl")" = 1
test "$(jq -s -r '[.[] | select(.row_key == "F002@1" and .source_expected == false and .facts.x == 1 and .facts.y == false)] | length' "$outdir/cases.jsonl")" = 1
test "$(jq -s -r '[.[] | select(.row_key == "F003@1" and .source_expected == false and .facts.x == 0 and .facts.y == 1)] | length' "$outdir/cases.jsonl")" = 1
test "$(jq -s -r '[.[] | select(.row_key == "F004@1" and .source_expected == true and .facts.flag == false)] | length' "$outdir/cases.jsonl")" = 1
