#!/usr/bin/env bash
# Verify the standard-new multi-backend writer and SHA-256 implementation.

set -euo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
standard="${STANDARD_NEW:-$root/../standard-new}"
expected_commit="ee61cfe"
outdir="${1:-$root/.cache/runs/E0025/R000001}"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
run_check focused_test fo test test_writer
run_check formatting fo fmt --check src/sha256.f90 src/writer.f90 test/test_writer.f90
...
printf 'file_bytes\taccepted\n' >>"$outdir/summary.tsv"
printf 'memory_bytes\taccepted\n' >>"$outdir/summary.tsv"
printf 'counting_size\taccepted\n' >>"$outdir/summary.tsv"
printf 'sha256_empty\taccepted\n' >>"$outdir/summary.tsv"
printf 'sha256_abc\taccepted\n' >>"$outdir/summary.tsv"
printf 'independent_oracle_mutation\tobserved_failure\n' >>"$outdir/summary.tsv"

if awk -F '\t' '$2 != 0 {failed=1} END {exit failed}' "$tmp/status.tsv"; then
    printf 'E0025 oracle: multi-backend writer passed all checks\n'
    cat "$outdir/summary.tsv"
else
    printf 'E0025 failed: one or more checks did not pass\n' >&2
    exit 1
fi
