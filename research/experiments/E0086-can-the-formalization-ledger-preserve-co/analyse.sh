#!/usr/bin/env bash
# Preserve competing deterministic interpretations as disputed records.
...
policy="$root/research/experiments/E0086-can-the-formalization-ledger-preserve-co/dispute-policy.tsv"
oracle="$root/research/experiments/E0086-can-the-formalization-ledger-preserve-co/independent-oracle.tsv"
outdir="${1:-$root/.cache/runs/E0086/R000001}"
...
test -f "$policy" || die 'dispute policy is missing'
...
printf 'constraint_no\tconstraint_id\tassociated_rules\tline\tpage\tstatus\tform\tsubject\tapplicability\trequired_facts\tprovided_facts\tpredicate_a\tpredicate_b\tsource_hash\torigin\tsource_text\tsource_evidence\treason\n' >"$outdir/formalizations.tsv"
awk -F '\t' -v OFS='\t' -v policy="$policy" -v source_hash="$source_hash" -v joined="$tmp/canonical-joined.txt" '
    ...
    }
    {
        ...
        if (id in policy_status) {
            ...
            predicate_b="-"
            evidence="-"
            reason="outside selected disputed slice"
        }
        print NR, id, associated, line, page, status, form, subject, applicability, required, provided, predicate_a, predicate_b, hash, origin, text, evidence, reason
    }
    END {
        if (policy_count != 3) bad=1
        for (id in policy_status) if (!(id in policy_seen)) bad=1
        exit bad
    }
' "$policy" "$constraints" >>"$outdir/formalizations.tsv" || die 'dispute policy did not match source evidence'
...
' "$outdir/fact-edges.tsv" >"$outdir/topological-order.tsv" || die 'dispute dependency graph contains a cycle'

awk -F '\t' -v OFS='\t' 'NR > 1 && $6 != "unresolved" {print $2, $6}' "$outdir/formalizations.tsv" | sort >"$tmp/actual-status.tsv"
awk -F '\t' -v OFS='\t' 'NR > 1 {print $1, $2}' "$oracle" | sort >"$tmp/oracle-status.tsv"
cmp -s "$tmp/actual-status.tsv" "$tmp/oracle-status.tsv" || die 'independent status oracle differs'
awk -F '\t' -v OFS='\t' 'NR > 1 && $6 != "unresolved" {print $2, $12, $13}' "$outdir/formalizations.tsv" | sort >"$tmp/actual-predicates.tsv"
awk -F '\t' -v OFS='\t' 'NR > 1 {print $1, $4, $5}' "$oracle" | sort >"$tmp/oracle-predicates.tsv"
cmp -s "$tmp/actual-predicates.tsv" "$tmp/oracle-predicates.tsv" || die 'independent predicate candidates differ'
awk -F '\t' -v OFS='\t' 'NR > 1 && $6 == "resolved" {print $2, $10}' "$outdir/formalizations.tsv" | sort >"$tmp/actual-required.tsv"
# Unresolved and disputed rows carry no accepted dependency facts.
awk -F '\t' -v OFS='\t' '$2 == "resolved" {print $1, $6}' "$oracle" | sort >"$tmp/oracle-required-all.tsv"
cmp -s "$tmp/actual-required.tsv" "$tmp/oracle-required-all.tsv" || die 'independent required-fact oracle differs'
awk -F '\t' -v OFS='\t' 'NR > 1 && $6 == "resolved" {print $2, $11}' "$outdir/formalizations.tsv" | sort >"$tmp/actual-provided.tsv"
awk -F '\t' -v OFS='\t' '$2 == "resolved" {print $1, $7}' "$oracle" | sort >"$tmp/oracle-provided.tsv"
cmp -s "$tmp/actual-provided.tsv" "$tmp/oracle-provided.tsv" || die 'independent provided-fact oracle differs'
awk -F '\t' -v OFS='\t' 'NR > 1 {print $2, $17}' "$outdir/formalizations.tsv" | sort >"$tmp/actual-evidence.tsv"
awk -F '\t' -v OFS='\t' 'NR > 1 {print $1, $3}' "$oracle" | sort >"$tmp/oracle-evidence.tsv"
# The unresolved remainder has no evidence field in the output.
awk -F '\t' '$2 != "-" {print}' "$tmp/actual-evidence.tsv" | sort >"$tmp/actual-evidence-selected.tsv"
cmp -s "$tmp/actual-evidence-selected.tsv" "$tmp/oracle-evidence.tsv" || die 'independent evidence oracle differs'
...
sed 's/DOUBLEPRECISION or the same as the name/DOUBLEPRECISION and the same as the name/' "$policy" >"$mutated_policy"
if grep -Fqi -- 'DOUBLEPRECISION and the same as the name' "$tmp/canonical-joined.txt"; then die 'negative disputed source mutation did not fail'; fi
...
awk -F '\t' -v OFS='\t' 'NR > 1 {print $1, $4, $5}' "$mutated_oracle" | sort >"$tmp/mutated-predicates.tsv"
if cmp -s "$tmp/actual-predicates.tsv" "$tmp/mutated-predicates.tsv"; then die 'negative disputed predicate mutation did not fail'; fi
...
test "$policy_rows" -eq 3 || die 'policy row count differs'
test "$resolved_constraints" -eq 2 || die 'resolved count differs'
test "$disputed_constraints" -eq 1 || die 'disputed count differs'
test "$unresolved_constraints" -eq 284 || die 'unresolved count differs'
test "$normalized_predicates" -eq 2 || die 'normalized predicate count differs'
...
test "$required_fact_records" -eq 6 || die 'required fact count differs'
test "$provided_fact_records" -eq 2 || die 'provided fact count differs'
test "$dependency_edges" -eq 8 || die 'dependency edge count differs'
...
printf 'E0086 oracle: disputed formalization preservation passed\n'
