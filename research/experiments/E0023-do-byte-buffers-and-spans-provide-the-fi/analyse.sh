#!/usr/bin/env bash
# Verify the first standard-new byte buffer and span implementation slice.

set -euo pipefail

root="$(cd "$(dirname "${BASH_SOURCE[0]}")/../../.." && pwd)"
standard="${STANDARD_NEW:-$root/../standard-new}"
expected_commit="f81fd52"
outdir="${1:-$root/.cache/runs/E0023/R000001}"
tmp="$(mktemp -d)"
trap 'rm -rf "$tmp"' EXIT
...
run_check focused_test fo test test_byte_text
run_check formatting fo fmt --check src/byte_buffer.f90 src/byte_span.f90 test/test_byte_text.f90
...
if awk -F '\t' '$2 != 0 {failed=1} END {exit failed}' "$tmp/status.tsv"; then
    printf 'E0023 oracle: byte buffer/span slice passed all checks\n'
    cat "$outdir/summary.tsv"
else
    printf 'E0023 failed: one or more checks did not pass\n' >&2
    exit 1
fi
