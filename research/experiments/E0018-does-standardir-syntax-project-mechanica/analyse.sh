...
(cd "$standard" && fo exec sxbison "$input" "$output")
...
grep -Fqx '    r_xyz h_R401_1' "$output"
grep -Fqx 'h_R401_1:' "$output"
grep -Fqx '  | h_R401_1 h_R401_2' "$output"
grep -Fqx 'h_R401_2:' "$output"
grep -Fqx '    "," r_xyz' "$output"
grep -Fqx 'r_program:' "$output"
grep -Fqx '    r_program_x2D_unit h_R501_1' "$output"

bytes="$(stat -c '%s' "$output")"
sha256="$(sha256sum "$output" | cut -d' ' -f1)"
printf 'E0018 oracle: %s records, %s Bison rules, %s helpers, ordered provenance agrees\n' \
    "$records" "$rules" "$helpers"
