...
M3 — twenty-two bounded semantic-oracle slices promoted; full Core 0 remains
pending
...
review. M3 is now open through twenty-two bounded contracts, including C717,
C720, C722, C724, C726, C731, C732, C733 and C735.
...
- M3 is `OPEN` through twenty-two promoted bounded slices, including C717,
  C720, C722, C724, C726, C731, C732, C733 and C735; full Core 0 remains open.
  D0126 selects
...
Current fixture: `T-M3-c743-derived-type-private-or-sequence-uniqueness-oracle`.
...
ID: `T-M3-c743-derived-type-private-or-sequence-uniqueness-oracle` — implement
the bounded C743 source-backed verifier/oracle selected by E0203/R000530.

Verifier: from a clean checkout, run `tests/e2e/run-m3-c743.sh --fresh`. The
oracle must classify the complete typed product of
`private-or-sequence-occurrence` (`none`, `single`, `duplicate`, `unknown`)
and `context` (`derived-type-def`, `other`, `unknown`) as
`ACCEPTED`/`REJECTED`/`UNRESOLVED`, bind C743 line 3637/page 89 to R726/R729,
reject source/page/rule/identity mutations, and record zero model calls and
zero semantic-fact promotions. It must not parse derived-type definitions,
resolve names or broaden C743/M3.
...
review R000491 pass. C724 replay R000494 and focused review R000495 pass, and
  ...
  twenty-two bounded slices promoted.
  The audit residual
...
The current blocker is witness closure outside the bounded slices: 149 rows
(85 disputed and 64 unwitnessed) remain after promoted-contract rows are
separated, as regenerated by R000529/R000530 and the post-C735 partition below.
C743@1 is now source-bound, but its bounded deterministic verifier/oracle is
...
Implement the C743 contract selected by D0149/E0203: typed occurrence and
context fields, a deterministic three-outcome oracle, positive and negative
neighbours, unresolved controls, mutation controls and replayable trace. The
exact central command is `tests/e2e/run-m3-c743.sh --fresh`. This is a bounded
oracle delivery only; it does not restart E0172, parse general Fortran or
close full M3.

The current post-C735 partition is regenerated with:
...
`T-M3-c743-derived-type-private-or-sequence-uniqueness-oracle`; E0203/R000530
has already selected C743 and the bounded verifier/oracle is the missing
artifact.
```
