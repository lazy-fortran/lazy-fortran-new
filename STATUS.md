...
- M3 is `OPEN` through the promoted C1106, C702, C601, C603 and C721 slices.
  ...
  Its exact replay is `tests/e2e/run-m3-c718.sh .cache/runs/E0182/R000001`;
  R000033 passes the central gate and focused review is pending.
...
Expected observable: a typed C718 candidate carrying named-constant and value-
type states whose deterministic oracle returns `ACCEPTED`, `REJECTED` or
`UNRESOLVED`, bound to C718 and the R709 StandardIR row, with two positive
...
Oracle: `tests/e2e/validate_m3_c718.py`, which computes the outcome from the
typed named-constant and value-type states independently of the expected
labels. Model output cannot promote a fact.
...
Verifier: `tests/e2e/run-m3-c718.sh .cache/runs/E0182/R000001` from clean
...
counters. Replay `R000033` passes; focused review is pending.
...
Run two focused independent reviews of the C718 candidate against R000033. If
...
review `R000015` are `PASS`; the C601 central replay `R000003` and focused
  ...
  .cache/runs/E0182/R000001` is `PASS` in R000033 and awaits focused review;
