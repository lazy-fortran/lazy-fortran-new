...
Expected observable: a typed label-spelling candidate whose deterministic
oracle returns `ACCEPTED`, `REJECTED` or `UNRESOLVED`, bound to C603 and the
R611 StandardIR row, with short, five-digit, all-zero, invalid-shape and source-
identity controls. The outcome inventory is generated by
`tests/e2e/run-m3-c603.sh .cache/runs/E0178/R000001`.

Oracle: `tests/e2e/validate_m3_c603.py`, which computes the outcome from the
typed label spelling independently of the expected labels. Model output cannot
promote a fact.

## Active task

ID: `T-M3-c603-semantic-oracle` — E0178 C603 source-backed semantic-oracle
vertical slice.

Verifier: `tests/e2e/run-m3-c603.sh .cache/runs/E0178/R000001` from clean
central and component checkouts. It includes the standard-new semantic-items
canonicalization, C603 source/provenance checks, the independent oracle, five
