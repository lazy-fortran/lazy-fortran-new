...
Expected observable: a typed C725 candidate carrying a kind-parameter state
whose deterministic oracle returns `ACCEPTED`, `REJECTED` or `UNRESOLVED`,
bound to C725 and the R723/R708 StandardIR rows, with two positive witnesses,
a negative neighbour, an unknown-state control and source-identity controls.
The outcome inventory is generated by
`tests/e2e/run-m3-c725.sh .cache/runs/E0180/R000001`.

Oracle: `tests/e2e/validate_m3_c725.py`, which computes the outcome from the
typed kind-parameter state independently of the expected labels. Model output
