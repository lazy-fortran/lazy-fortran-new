...
- M3 is `OPEN` through `T-M3-c1106-semantic-oracle`. D0124 selects the
  source-backed C1106 associate-construct-name relation. Its exact gate is
  `tests/e2e/run-m3-c1106.sh`; before its first clean replay no semantic fact
  is promoted and the full Core 0 milestone remains open.
...
oracle; clean replay is pending.

Boundary decision: `research/decisions/D0124-first-m3-associate-name-oracle.md`.
...
`tests/e2e/run-m3-c1106.sh .cache/runs/E0175/R000469`.
...
Verifier: `tests/e2e/run-m3-c1106.sh .cache/runs/E0175/R000469` from clean
...
remaining blocker is only the clean replay and focused review for the new
bounded slice; a green slice does not close full M3.

## Next executable task

Run the exact bounded gate:
...
fact. After the slice gate and focused review, define a separate next M3
contract before broadening.
...
central commit `bdb8717`. The M3 C1106 contract is staged; its clean replay
and focused review are the next verification step.
```
