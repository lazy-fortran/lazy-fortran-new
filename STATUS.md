...
declaration/execution envelopes now carry counted v2 execution-part identities
for counts 2 through 10, with matching stack slots and exits 8 through 16.
Fortback generates the count-specific v2 routes from one compact
storage-sequence pattern; the separate old v1 route remains distinct. This
remains bounded sequence transport, not general statement parsing, name
resolution, arbitrary storage or full M3 semantics. The source-backed `STOP 7`, `PRINT *, 7`,
...
stored-variable lists. Fortfront now derives stored-variable list items from
the existing initializer parser, and FFC has removed the remaining fixed
three-item `17, 18, 19` route. The clean central gate
...
with central witnesses for zero and one hundred, through AST-v2, typed MIR,
RISC-V and qemu. The exact central gate is
`bash tests/e2e/check-generated-print-expression-decimal-constant-zero.sh`;
it passes both positives, five source neighbours, the prior decimal slice and
...
output lists with one, four and ten literal items reach AST-v2, typed MIR,
RISC-V and qemu, while an eleven-item list and AST/MIR/ELF mutations reject.
...
The positive-initializer edge successor is now integrated as a bounded
extension: `bash tests/e2e/check-generated-print-variable-positive-initializer.sh`
checks `x = 0` and `x = 2047` through AST-v2, typed MIR, RISC-V and qemu,
rejects `x = 2048` and real initialization, and rejects AST/MIR/ELF mutations.
This completes only the generated positive policy edges for this stored-
variable witness; general assignment parsing and semantic promotion remain
open.
The generic-initializer successor now passes
`bash tests/e2e/check-generated-print-variable-generic-initializer.sh`: novel
`x = 42` and `x = -42` sources reach AST-v2, typed MIR, RISC-V and qemu,
while `2048`, `-101`, real initialization, wrong PRINT names and artifact
mutations reject. The frontend and generated lowering/backend policies now
transport the bounded integer initializer range without one branch per value.
This remains one stored-variable/PRINT shape, not general assignment parsing,
name resolution or semantic promotion.
The initialized-add successor now passes
`bash tests/e2e/check-generated-print-variable-generic-add.sh`: novel
`x = 42; x = x + 1` and `x = -42; x = x + 1` sources reach AST-v2, nine-
instruction MIR, RISC-V and qemu with exact `43\n` and `-41\n` output;
wrong operator, right operand, PRINT name, real initializer and AST/MIR/ELF
mutations reject. The frontend preserves one source identity across the
envelope and assignment sequence, so FFC's independent provenance validation
is exercised. This remains one initialized arithmetic shape, not general
expression parsing, integer semantics, name resolution or semantic promotion.
The bounded-addend successor now passes
`bash tests/e2e/check-generated-print-variable-generic-addend.sh`: novel
`x = 42; x = x + 2` and `x = -42; x = x + 10` sources reach AST-v2, exact
nine-instruction MIR, RISC-V and qemu with `44\n` and `-32\n` output. Addends
outside the generated 1..10 policy, malformed source neighbours and
AST/MIR/ELF mutations reject. This extends one initialized-add shape; it does
not provide general expressions, name resolution or semantic promotion.
The bounded-subtrahend successor now passes
`bash tests/e2e/check-generated-print-variable-generic-subtrahend.sh`: novel
`x = 42; x = x - 2` and `x = -42; x = x - 10` reach AST-v2, exact
nine-instruction MIR, RISC-V and qemu with `40\n` and `-52\n` output. Addends
outside the generated 1..10 policy, malformed source neighbours and
AST/MIR/ELF mutations reject. ASCII source `-` is normalized at the FFC AST
boundary; this remains bounded initialized arithmetic, not general expression
parsing or semantic promotion.
The bounded-multiplier successor now passes
`bash tests/e2e/check-generated-print-variable-generic-multiplier.sh`: novel
`x = 42; x = x * 3` and `x = -42; x = x * 10` reach AST-v2, exact
nine-instruction MIR, RISC-V and qemu with `126\n` and `-420\n` output.
Multiplier zero/eleven, real and malformed source neighbours, and AST/MIR/ELF
mutations reject. The runner directly replays the existing addend and
subtrahend routes; this remains bounded initialized arithmetic, not general
expression parsing or semantic promotion.
The bounded-divisor successor now passes
`bash tests/e2e/check-generated-print-variable-generic-divisor.sh`: novel
`x = 42; x = x / 2` and `x = -42; x = x / 10` reach AST-v2, exact
nine-instruction MIR, RISC-V and qemu with `21\n` and `-4\n` output. Divisor
zero/eleven, real and malformed source neighbours, and AST/MIR/ELF mutations
...
The initialized variable-power successor now passes
`bash tests/e2e/check-generated-print-variable-generic-power-variable.sh`:
`x = 3; x = x ** x` and `x = 4; x = x ** x` reach AST-v2, exact
`const/store/load/load/pow/store/load/output/return` MIR, generated RISC-V and
qemu with `27` and `256`. Negative, real, wrong-name, wrong-operator and
wrong-RHS neighbours, plus AST/MIR/ELF mutations, reject. This is a bounded
positive-initializer witness for variable power, not arbitrary variable-
exponent semantics, general expression parsing or semantic promotion.
The initialized variable-add successor now passes
`bash tests/e2e/check-generated-print-variable-generic-variable-add.sh`:
`x = 3; x = x + x` and `x = 4; x = x + x` reach AST-v2, exact
`const/store/load/load/add/store/load/output/return` MIR, generated RISC-V and
qemu with `6\n` and `8\n`. A signed out-of-range initializer, real initializer,
wrong-name/operator/RHS neighbours, and AST/MIR/ELF mutations reject. This is
a bounded initialized variable-add witness, not general expression parsing or
semantic promotion.
The initialized variable-multiply successor now passes
`bash tests/e2e/check-generated-print-variable-generic-variable-multiply.sh`:
`x = 3; x = x * x` and `x = 4; x = x * x` reach AST-v2, exact
`const/store/load/load/mul/store/load/output/return` MIR, generated RISC-V and
qemu with `9\n` and `16\n`. A signed out-of-range initializer, real initializer,
wrong-name/operator/RHS neighbours, and AST/MIR/ELF mutations reject. This is
a bounded initialized variable-multiply witness, not general expression
parsing or semantic promotion.
The initialized variable-divide successor now passes
`bash tests/e2e/check-generated-print-variable-generic-variable-divide.sh`:
`x = 3; x = x / x` and `x = -4; x = x / x` reach AST-v2, exact
`const/store/load/load/div/store/load/output/return` MIR, generated RISC-V and
qemu with `1\n` and `1\n`. A signed out-of-range initializer, real initializer,
wrong-name/operator/RHS neighbours, and AST/MIR/ELF mutations reject. This is
a bounded initialized variable-divide witness, not general expression parsing
or semantic promotion.
The initialized variable-subtract successor now passes
`bash tests/e2e/check-generated-print-variable-generic-variable-subtract.sh`:
`x = 3; x = x - x` and `x = -4; x = x - x` reach AST-v2, exact
`const/store/load/load/sub/store/load/output/return` MIR, generated RISC-V and
qemu with `0\n` and `0\n`. A signed out-of-range initializer, real initializer,
wrong-name/operator/RHS neighbours, and AST/MIR/ELF mutations reject. This is
a bounded initialized variable-subtract witness, not general expression
parsing or semantic promotion.
The source-derived variable-y initializer successor now passes
`bash tests/e2e/check-generated-print-variable-generic-variable-y-initializer.sh`:
`y = 3` and `y = -4` reach AST-v2, exact `const/store/load/output/return` MIR
with storage key `y`, generated RISC-V and qemu with `3\n` and `-4\n`. An
out-of-range value, real declaration, wrong declaration/use, and AST/MIR/ELF
mutations reject. This is a bounded source-derived variable-name witness, not
general name resolution or semantic promotion.
The source-derived variable-z initializer successor now passes
`bash tests/e2e/check-generated-print-variable-generic-variable-z-initializer.sh`:
`z = 5` and `z = -6` reach AST-v2, exact `const/store/load/output/return` MIR
with matching storage key `z`, generated RISC-V and qemu with `5\n` and `-6\n`.
An out-of-range value, real declaration, wrong declaration/use, and AST/MIR/ELF
mutations reject. This is a bounded source-derived variable-name witness, not
general name resolution or semantic promotion.
...
The follow-on `T-L3-generic-print-list-literals` slice now passes
`bash tests/e2e/check-generated-print-list-literals.sh`: mixed `PRINT` lists
with novel nonnegative decimal literals (`x, 20, 21` and `x, 100, 200, 300,
400`) pass the independent AST/MIR/QEMU oracle, four source neighbours reject,
and AST/MIR mutation controls reject. Frontend admission, FFC lowering tests,
and fortback multi-digit decimal emission are pinned in `repos.toml`. This is
still list-directed integer output with one stored variable; signed
...
with source-backed R1006/R1009 evidence and an independent oracle for `x / 2`
as one generic PRINT item. The division component implementation and central
replay now pass; the
...
The L3 slice `T-L3-generic-print-expression-power-variable-value` is now
integrated: frontend, FFC, and backend transport `x = 4` through the same
source-backed `x ** x` shape, and the central oracle passes exact `256` in both
list positions while preserving the promoted `x = 3` result `27`. The baseline
failure was useful: it exposed initializer rejection, then initializer loss in
...
The L3 add-constant successor is now integrated: ordinary ASCII `x + 2` in a
generic PRINT list lowers through generated AST, typed MIR
`load/const/add/output`, RISC-V and qemu. The central command
`bash tests/e2e/check-generated-print-expression-add-constant.sh` passes both
list positions, five rejected source neighbours, the prior expression chain,
and AST/MIR/ELF mutation controls. The earlier multiply verifier's obsolete
`x + 2` negative neighbour was replaced by `x + 3` when this capability became
valid. General expressions, formatted I/O, arrays, non-integer output and
semantic promotion remain out of scope.

The canonical ASCII-minus successor is now integrated: ordinary Fortran
`x - 2` in a generic PRINT list lowers through generated policy, typed MIR
`load/const/sub/output`, the existing RISC-V sub route and qemu, while the
Unicode en-dash witness remains accepted. The central command
`bash tests/e2e/check-generated-print-expression-ascii-subtract.sh` passes both
list positions, five rejected source neighbours, the prior expression chain,
and AST/MIR/ELF mutation controls. The earlier en-dash verifier's obsolete
ASCII `x - 2` negative was replaced by `x - 3`; general expressions, formatted
I/O, arrays, non-integer output and semantic promotion remain out of scope.
...
`2d32e309d29c49d6e51bfeff0621e4fd0f5ec91e`, and `fortback-new`
`5753043beff6d66279b1888bb931ecd5e51ce53f`. Their full `fo` gates pass, and
...
item count instead of historical source names, its batch PRINT span derives
from the recognized source line instead of a per-count ladder, and its bounded
assignment sequence assembly reuses one typed repeated-assignment path with
generic span calculation. Its generated PRINT policy validates legal variable names and the
...
uses that generic path directly. Its large stored-variable PRINT emitters now
share one count-driven dispatch while preserving range-specific generated facts.
Its four variable-binary expression recognizers now share one parameterized
...
Fortback now derives the repeated generated
PRINT route-operation pattern from a compact rule with boundary controls while
retaining explicit route facts, its stored-variable PRINT matcher plus one
count-driven storage-sequence encoder share generic paths, and its stored-variable
PRINT encoder uses one loop for counts three through one hundred with a distinct
two-item prologue. FFC validates the AST-v2
...
uses one count-driven check for routes 19 through 28, its variable PRINT
counts 2 through 10 use one parameterized emitter and validator, and its ten
legacy literal-list emitters share one constructor, and its four bounded
...
central replay's assignment routes for counts 3 through 10 now share the existing
runner and use program-unit-v2, while count 2 retains its stricter negative control. Its pure literal
PRINT routes for counts 2 through 10 now share one replay helper, and its
...
Its assignment-sequence oracle uses one count-derived check for counts 2 through
10, checking counted v2 source rules for counts 3 through 10 and retaining only
the count-two source-rule exception. The raw scalar
...
Fortfront's generic initialized-update parser now handles the existing
multiply, divide, power and Unicode en-dash subtraction expression fixtures
without source-specific fallback construction. Its bounded 2--10 assignment
parser now derives each assignment and span from the source envelope instead
of a source-and-count ladder. Broader expression and statement parsing remain
open. Its SX serializer now emits every accepted assignment count through one
bounded loop instead of a branch per count. Its AST-v1 assignment-sequence
...
sequences remain open.
The raw scalar initialized-division route now carries `counter_2 = 42` and
`counter_2 = counter_2 / 2` through the same AST-v2, MIR, backend, and qemu
path, with exact `21` output. General expression parsing and assignment
sequences remain open.
...
| ffc-new | lazy-fortran/ffc-new | `2d32e309d29c49d6e51bfeff0621e4fd0f5ec91e` | compiler driver and middle end | source-order opcode-histogram table, generic expression lowering, generic PRINT-list traversal, wrapper-free parameterized initialized arithmetic MIR paths, count-driven legacy PRINT route matching, parameterized variable PRINT emission for counts 2 through 10, one count-driven dispatch for large stored-variable PRINT emitters, shared literal-list emission, one parameterized bounded binary-expression parser, shared variable-binary lowering, one parameterized initialized literal-binary path, one parameterized AST-v2 literal-list validator, direct generic AST-v1 assignment-sequence lowering, generic AST-v2 raw assignment-sequence lowering, count-driven AST-v1 assignment-sequence parsing, generic raw scalar initialized-add, subtraction, multiplication and division lowering, one parameterized variable-binary expression recognizer, one shared assignment-expression serializer retaining four raw exceptions, one parameterized multiply/divide/subtract AST-v1 shape validator, one parameterized two-instruction scalar-program shape validator, one parameterized no-assignment scalar lowering helper, counted AST-v2 execution-part identities for the bounded assignment range, root-identity AST-v2 assignment provenance for the bounded sequence range, with `fo` passing |
| fortback-new | lazy-fortran/fortback-new | `5753043beff6d66279b1888bb931ecd5e51ce53f` | backend | generated result-shape fact lookup plus generic encoding for pure-literal and stored PRINT lists, one count-driven variable PRINT matcher covering the bounded item range, one count-driven variable PRINT validator, shared initialized-expression validation, compact generated PRINT route-operation facts with boundary controls, one count-driven storage-sequence encoder, counted AST-v2 storage-sequence route facts generated from one compact pattern, one loop-based stored-variable PRINT encoder for counts three through one hundred with a distinct two-item prologue, generic legal scalar storage validation for initialized expressions, and counter subtraction, multiplication and division regression coverage, with `fo clean && fo && fo test --all` passing |
