...
existing StandardIR R721. Full M3 remains open; do not resume E0172 or start
broad semantic work.

## Active task

ID: `T-M3-c731-constant-expression-oracle` — define and implement the bounded
C731 constant-expression oracle. Full M3 remains open.

Verifier: `tests/e2e/run-m3-c731.sh --fresh` after D0145/E0196 are defined. It
must bind C731 lines 3469--3470 on page 85 to existing R721, use typed
constant-expression/context states and observe zero model calls or semantic
promotions.
...
Run and record `T-M3-c731-constant-expression-oracle`. First define D0145 and
E0196 from the C731 selection report, then run the exact central verifier:

```text
tests/e2e/run-m3-c731.sh --fresh
```

The bounded oracle must use only the pinned C731 source span and existing R721
shape, must not evaluate arbitrary Fortran expressions or infer names, and
