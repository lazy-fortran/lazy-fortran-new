...
Current fixture: `T-M3-c745-sequence-component-presence-oracle`.
...
implements this bounded relation only; full M3 remains open.
...
presence (`absent`, `present`, `unknown`), component presence (`zero`,
`one-or-more`, `unknown`) and context (`derived-type-def`, `other`, `unknown`)
as `ACCEPTED`/`REJECTED`/`UNRESOLVED`, bind C745 lines 3665--3667/page 89 to
R726/R731/R735, reject source/page/rule/identity mutations, and record zero
model calls and zero semantic-fact promotions. It must not parse definitions,
count real components or evaluate the other C745 obligations.
...
obligation; its bounded deterministic verifier/oracle is not implemented yet.
A
...
positive and negative neighbours, unresolved controls, mutation controls and
replayable trace. The exact central command is
