...
| fortfront-new | lazy-fortran/fortfront-new | `14fb15010a4566eec87c5192923aa442fc0f8eaf` | frontend | generated program-unit-v2 CLI, bounded assignment sequences, generic stored-variable initializer transport across generated -100..2047 integer policy values including 42 and -42, generic initialized `x = x + n`, `x = x - n` and `x = x * n` transport for bounded n=1..10 with consistent source provenance, generic initialized `x = x / n` transport for bounded nonzero n=1..10, STOP 7, repeated PRINT items, generic mixed integer output-list AST-v2 with source-driven 1..10 cardinality, bounded signed integer PRINT literals, bounded x=3/x=4 variable power expressions, source-backed generic x–2, x+2 and canonical ASCII x-2 expression items, and bounded decimal x+n/x-n constants including zero through one hundred; focused gates pass |
| ffc-new | lazy-fortran/ffc-new | `48237870a118830befa3cae3aef1141cb5eeb32d` | compiler driver and middle end | generated v2 assignment envelopes, sequence MIR routes, generic mixed integer output-list lowering with enforced 1..10 cardinality, generated stored-variable initializer const/store transport across -100..2047 including 42 and -42, generic initialized `x = x + n`, `x = x - n` and `x = x * n` nine-instruction MIR lowering for bounded n=1..10, generic initialized `x = x / n` nine-instruction MIR lowering for bounded nonzero n=1..10, bounded signed integer PRINT literal values, positive stored-variable initializer transport, x=3/x=4 load/load/pow plus decimal power-expression lowering, and bounded load/const/add-or-sub/output for decimal x+n/x-n constants including zero through one hundred with preserved AST provenance; focused multiplier/divisor/subtraction gates pass; full `fo` retains unrelated existing failures |
| fortback-new | lazy-fortran/fortback-new | `89a9b6a1cc218cbf6fd680734e9626c403447b3b` | backend | generated stack-slot/sequence routes, generic mixed and literal-only integer output-list lowering with 1..10 cardinality, generated stored-variable PRINT emission across -100..2047 including 42 and -42, generic initialized `x = x + n`, `x = x - n` and `x = x * n` RISC-V/QEMU emission for bounded n=1..10, generic initialized `x = x / n` RISC-V/QEMU emission for bounded nonzero n=1..10, bounded x=3/x=4 x**x plus integer-power decimal emission, generic load/const/sub/output plus x+2 add emission, and bounded decimal add/sub constants including zero through one hundred; focused gates pass; generated-policy formatting warning remains |
