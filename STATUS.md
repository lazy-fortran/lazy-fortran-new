...
M3 — bounded C601 semantic-oracle slice promoted; full Core 0 remains pending
...
- M3 is `OPEN` through the promoted C1106, C702 and C601 slices. D0126 selects
  ...
  focused review `R000022` pass. No model output is promoted and full Core 0
  remains open.
...
ID: `T-M3-c601-semantic-oracle` — bounded source-backed C601 name-length
semantic oracle; central replay `R000003` and focused review `R000022` pass.
The bounded slice is promoted; full M3 remains open.

Boundary decision: `research/decisions/D0127-third-m3-c601-name-length-oracle.md`.

Expected observable: a typed name-spelling candidate whose deterministic
oracle returns `ACCEPTED`, `REJECTED` or `UNRESOLVED`, bound to C601 and the
R601/R602/R603 StandardIR rows, with short, boundary, over-limit, invalid-shape
and source-identity controls. The outcome inventory is generated by
`tests/e2e/run-m3-c601.sh .cache/runs/E0177/R000003`.

Oracle: `tests/e2e/validate_m3_c601.py`, which computes the outcome from the
typed spelling independently of the expected labels. Model output cannot
promote a fact.

## Active task

ID: `T-M3-c601-semantic-oracle` — E0177 C601 source-backed semantic-oracle
vertical slice.

Verifier: `tests/e2e/run-m3-c601.sh .cache/runs/E0177/R000003` from clean
central and component checkouts. It includes the standard-new semantic-items
canonicalization, C601 source/provenance checks, the independent oracle, five
...
review R000022 pass. The remaining blocker is full M3/Core 0, which requires a
fourth separately defined bounded contract; a green slice does not close full
M3.

## Next executable task

Define the next separate, source-backed M3 contract over an already represented
shape after C601. Do not resume E0172, start broad parsing/semantic work, or
promote a model fact. Do not treat the promoted C601 slice as the full M3
milestone.
...
review `R000022` are `PASS`. The bounded C1106, C702 and C601 slices are
