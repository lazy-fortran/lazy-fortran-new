...
M3 — eight bounded semantic-oracle slices promoted; C729 candidate pending focused review; full Core 0 remains pending
...
only that bounded slice. D0133 selects C729 as the next bounded contract; its
replay passes and focused review is pending. The full Core 0 semantic
...
- M3 is `OPEN` through the promoted C1106, C702, C601, C603 and C721 slices.
  ...
  R000042 passes and focused review is pending.
...
ID: `T-M3-c729-semantic-oracle` — bounded source-backed C729 optional-comma
semantic oracle; replay `R000003` passes and focused review is pending.
C723 remains promoted; full M3 remains open.

Boundary decision: `research/decisions/D0133-ninth-m3-c729-optional-comma-oracle.md`.

Expected observable: a typed C729 candidate carrying comma and context states
whose deterministic oracle returns `ACCEPTED`, `REJECTED` or `UNRESOLVED`,
bound to C729 and the R722/R703/R801 StandardIR rows, with two positive
witnesses, a negative neighbour, an unknown-state control and source-identity
controls.
The outcome inventory is generated by
`tests/e2e/run-m3-c729.sh --fresh`.

Oracle: `tests/e2e/validate_m3_c729.py`, which computes the outcome from typed
comma and context states independently of the expected labels. Model output
cannot promote a fact.

## Active task

ID: `T-M3-c729-semantic-oracle` — E0184 C729 source-backed semantic-oracle
vertical slice; central replay `R000042` passes and focused review is pending.
The candidate is bounded to this slice; full M3 remains open.

Verifier: `tests/e2e/run-m3-c729.sh --fresh` from clean
central and component checkouts. It includes the standard-new semantic-items
canonicalization, C729/R722/R703/R801 source/provenance checks, the independent
oracle, five mutation controls, exact trace comparison and zero model/promotion
counters. Replay `R000042` passes; focused review is pending.
...
C723 replay and focused review are green. The C729 replay is green but focused
review is pending; neither bounded slice can close the complete ledger gate. A
...
Run two focused independent reviews of the C729 candidate against R000042. If
both pass, promote only this bounded slice; then reassess the full M3/Core 0
boundary using the E0181 audit and the nine bounded slices. Do not resume
E0172, start broad parsing/semantic work, or promote a model fact.
...
review `R000015` are `PASS`; the C601 central replay `R000003` and focused
  ...
  and awaits focused review; eight bounded slices are promoted and full M3
  remains open.
```
